...
**The Qwen3.8-Flash-Next preset.** `qwen3-8-flash-next-nvfp4` serves Qwen3.8-Flash-Next (a 125B mixture of experts with 6B active, a 51B n-gram embedding table, 262k context, tools, reasoning, vision) on one 128 GB unified-memory Blackwell node from an OCI model image (giantswarm/agent-platform#553). NVIDIA's NVFP4 checkpoint is 123.6 GiB on disk and does not fit the node next to a KV cache, so the preset serves the quantization-aware-distilled repack `local-inference-lab/Qwen3.8-Flash-Next-NVFP4` (98.6 GiB, the n-gram table in NVFP4 too) with the table read from disk (`VLLM_PLE_TABLE_MEMORY=disk`) and MTP speculative decoding (four draft tokens): `weightsGiB: 99` (the Hub's size), `overheadGiB: 20` (about 74 GiB resident; `--gpu-memory-utilization=0.80` of the 128 GB leaves the rest to the fp8 KV cache), eight concurrent requests at 4096 batched tokens, the `qwen3` reasoning parser, tools on with the `qwen3_xml` parser. It is the first shipped preset served from a model image. Its serving image is the B12X serving stack — the upstream vLLM image cannot serve this architecture on the node — which the installation names as the runtime image of the well-known `LLMInferenceServiceConfig` such a node serves with (`kserve-runtime-configs`, [The well-known configs](#the-serving-slice-and-the-models-gateway)); the preset itself names no image. `spec.model.storageUri` is `oci://gsoci.azurecr.io/giantswarm/models/qwen3-8-flash-next-nvfp4:7c4f1bc1a2d6` (the tag is the first 12 characters of the checkpoint revision); `modelServing.modelImages.registry` swaps the host for an installation's own registry that holds the same path, and `modelServing.prepull.modelPresets` pre-pulls the image onto the serving nodes. `HOME`, `HF_HOME` and `VLLM_CACHE_ROOT` sit under `/tmp` because the modelcar path runs the runtime as a non-root uid on a read-only image. `make verify-wiring` expects thirteen shipped presets; `verify-preset-weights` and the eval check of `verify-serving-slice` cover the file; `verify-gpu-pool` leaves the preset's ConfigMap and its discovery entry out of the golden comparison while `origin/main` predates it.

**The four-GPU presets.** `mistral-small-4` and `gpt-oss-120b` are the first presets on more than one GPU (giantswarm/agent-platform#591): four 48 GB L40S of a `g6e.12xlarge` (48 vCPU, 384 GiB) — the smallest curated size with more than one L40S; the family has no two-GPU size — with the model split across the four by tensor parallel. Nothing in the chart or in model-manager derives it: `resources.gpus: 4` is requested and limited as `modelServing.serving.gpuResourceName` on the one model pod, and the preset's own `--tensor-parallel-size=4` is what makes vLLM shard the model — the well-known template adds a `--tensor-parallel-size` of its own only for an `LLMInferenceService` with `spec.parallelism.tensor`, which model-manager never sets, so the preset's flag is the only one on the command line; `verify-serving-slice` holds every shipped preset's `resources.gpus` equal to the tensor-parallel size its arguments set (1 without the flag). Both are served from OCI model images (`giantswarm/models/mistral-small-4-nvfp4:45331841b631`, `giantswarm/models/gpt-oss-120b:b5c939de8f75`; the tag is the first 12 characters of the checkpoint revision) with `HOME`, `HF_HOME` and `VLLM_CACHE_ROOT` under `/tmp`, 128k context, eight concurrent requests, tools and reasoning on (the `mistral` parsers; `openai` and `openai_gptoss`), requests 16 vCPU / 96 GiB and limits 32 / 160 GiB. Memory is judged on the cards a preset requests: the fit check budgets 4 × 48 = 192 GiB against `weightsGiB + overheadGiB` — Mistral Small 4 NVFP4 66 + 60 = 126 GiB (16.5 GiB of weights per card, the rest of `--gpu-memory-utilization=0.90` the KV cache), gpt-oss-120b 61 + 40 = 101 GiB (15.2 GiB per card). Ada has no native FP4: vLLM runs Mistral's NVFP4 (W4A16) and gpt-oss's MXFP4 through Marlin, the path `qwen3-8-27b-l40s` runs on today; Mistral's MLA attention is pinned to `TRITON_MLA` as its model card does (FlashMLA is Hopper-only), and its checkpoint is read in the mistral format (`--tokenizer-mode`, `--config-format`, `--load-format`). The two model images are giantswarm/models' to build and sign (its issue 7); the presets ship ahead of them and the first serve of each on the platform is the proof that the well-known template's 1 GiB `/dev/shm` carries four tensor-parallel workers.
