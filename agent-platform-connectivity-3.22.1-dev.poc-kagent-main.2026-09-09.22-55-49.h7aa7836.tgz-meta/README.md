...
## Dev channel (kagent API v2 line)

**What this branch is.** `poc/kagent-main` is the development line of the Agent Platform on kagent `main` — kagent's API v2 (`kagent.dev/v1alpha3`: `AgentTemplate`, `Harness`, `RemoteMCPServer`, `ModelConfig`; agents run as [Agent Substrate](https://github.com/kagent-dev/substrate) actors in gVisor worker pods instead of per-agent Deployments). It is where Team Bumblebee works on that version together until it is ready to release; it is not merged to `main` while the fleet runs the kagent 0.2.x wrapper (kagent 0.10, v1alpha2). PR [#330](https://github.com/giantswarm/agent-platform/pull/330) is the running review; the plan is `agent-v1alpha3-migration` in bumblebee-plans.

**What it publishes.** Every push to the branch runs the branch pipeline and, once the tests are green, publishes **both charts** to gsoci as dev builds (`push-chart` and `push-connectivity-chart`, the RFC `semver-based-automatic-upgrades` dev channel): `oci://gsoci.azurecr.io/charts/giantswarm/agent-platform` and `…/agent-platform-connectivity`, each tagged

```
<next patch of the newest stable tag>-dev.poc-kagent-main.<YYYY-MM-DD>.<HH-MM-SS>.h<sha7>
e.g. 3.22.1-dev.poc-kagent-main.2026-09-10.08-12-33.h7f841be
```

— gitsemver's live schema (v2.0.1: the branch lowercased to `[a-z0-9-]`, the **committer date** of the commit, so the two charts of one commit carry the **identical version**; `helm show chart` of both shows it). The prerelease identifiers sort chronologically, so the highest tag matching the branch is its newest build. The charts also land in the `giantswarm-test-catalog`. The sibling POC branches publish the same way: `giantswarm/backstage`, `giantswarm/model-manager`, `giantswarm/agent-manager` (`poc/kagent-main` each, charts and images on gsoci) and the kagent fork (`poc/agent-platform`, charts and images under `ghcr.io/giantswarm/kagent/…`, tagged `0.11.0-dev.poc-agent-platform.…`).

**How the meta chart selects it.** A dev-channel component carries a prerelease-admitting `versionRange` and a `semverFilter` — the branch's tag pattern, applied by Flux to the registry's tags *before* the range (see [Meta-package release flow](#meta-package-release-flow)). This branch's `values.yaml` defaults do that for its siblings (`components.agent-platform-connectivity` `">=3.0.0-0 <4.0.0-0"`, `components.backstage` / `model-manager` / `agent-manager` `">=0.0.0-0 <1.0.0-0"`, all with `".*-dev\\.poc-kagent-main\\..*"`; `components.kagent` and `components.kagent-crds` `">=0.11.0-0 <0.12.0-0"` with `".*-dev\\.poc-agent-platform\\..*"` from `oci://ghcr.io/giantswarm/kagent/helm`; `gitops.self.semverFilter` for the chart itself), so **a consumer selects only the meta chart's dev channel** and every sibling follows its own branch's newest build. The filter constant is defined once, in the `components:` preamble of [`helm/agent-platform/values.yaml`](helm/agent-platform/values.yaml); `make verify-components` asserts every copy agrees. Two things a consumer sets next to `components.kagent.enabled: true`: `components.kagent-crds.enabled: true` (kagent `main` ships its CRDs as their own chart — the render refuses kagent on without it) and nothing for `kagent.serviceMonitor` (the default is `false` here: kagent `main` serves no metrics listener). Two keys the stable line had are gone with the Kyverno mutations and fail the render if a values file still carries them: `kyvernoPolicies.seccompPolicyName`, `kyvernoPolicies.seccompRuleNames`.

**Consuming it in agentlab.** The lab follows the branch:

```bash
agentlab self-update
mkdir dev-lab && cd dev-lab
agentlab configure --defaults --chart-branch poc/kagent-main   # resolves the newest dev tag, turns Substrate on
...
**agent-manager's values on this line.** The agent-manager dev build writes agents as `kagent.dev/v1alpha3` `AgentTemplate` CRs (plus their carrier `RemoteMCPServer`) directly, as the caller — there is no agent chart and no agent `HelmRelease` — and its chart's `values.schema.json` is closed without `agentChart` and without `flux`. The meta chart therefore forwards neither: the release line's `agent-manager.agentChart` (`ociUrl`, `semver`) is not set in this branch's `values.yaml`, and the derivation of `agent-manager.flux.helmReleaseServiceAccount` from `kagent.fluxServiceAccountName` (`agent-platform.componentDerivedValues`) is off; a values file that still sets either key fails the render, naming the key. The tenant identity itself (ServiceAccount + RoleBinding `kagent-flux`, the portal's `agentPlatform.fluxServiceAccountName`) stays rendered. `make verify-components-charts` renders the forwarded `agent-manager` and `model-manager` blocks against the charts the dev channel resolves to — a schema violation fails the target — and `make verify-identity` / `make verify-managers` assert the two keys are absent and refused. The release of this line restores both with the agent chart.
...
spec:
  ...
  ref:
    semver: ">=3.0.0-0 <4.0.0-0"           # a bound without -0 never matches a prerelease; Flux rejects the RFC's "*-*"
...
**The release of this line resets all of it.** When this line ships, every `semverFilter` goes, the ranges return to the stable lines (`agent-platform-connectivity` `">=1.0.0"`; `backstage`, `model-manager`, `agent-manager` `"0.x"`; kagent and kagent-crds to the GS wrapper's line on gsoci), `gitops.self.semverFilter` to `""`, the ATS smoke turns kagent and agent-manager back on and `GOLDEN_REF` in `Makefile.custom.mk` back to `origin/main`. `CHANGELOG.md` carries the same rule; the customer BOM example gets its stable pins back.
