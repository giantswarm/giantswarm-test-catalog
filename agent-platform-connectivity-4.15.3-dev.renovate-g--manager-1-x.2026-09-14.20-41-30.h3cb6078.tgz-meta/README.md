...
**Drift detection.** The kagent HelmRelease carries `spec.driftDetection.mode: enabled` (`components.kagent.driftDetection`; a passthrough every component entry takes, set for kagent only): on every reconcile — `gitops.interval` — helm-controller compares the release manifest with the cluster and re-applies what differs or is missing, as a server-side apply without a Helm revision. So the platform Harness — deleted by the 4.8.0 upgrade of a consumer whose pinned connectivity chart skipped 4.7.19's `keep`, and never recreated by a release without drift detection (a plain reconcile of an unchanged release reports "in-sync") — is back within one interval and every template it admits returns to Ready (giantswarm/agent-platform#409; the ATS smoke deletes it and sees it back). A hand edit to an object of the release is reverted the same way; an object opts out with the annotation `helm.toolkit.fluxcd.io/driftDetection: disabled`.
