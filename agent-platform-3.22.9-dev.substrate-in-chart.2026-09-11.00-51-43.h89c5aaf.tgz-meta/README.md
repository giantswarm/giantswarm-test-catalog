...
| `agent-platform-connectivity` | the consumer-side **wiring** the meta-package renders as a child release: the public muster route, the agentgateway data-plane `Gateway` + `AgentgatewayParameters` + `HTTPRoute`s + `BackendTrafficPolicy`s, the `NetworkPolicy`s, the kagent/klaus-gateway/model-manager/agent-manager routes, the admin-owned kagent catalog (`ModelConfig`s and operator `RemoteMCPServer`s at `kagent.dev/v1alpha3`; the per-agent muster `RemoteMCPServer` is the agent chart's), the `kagent-flux` tenant identity, and the CNPG `Cluster`. Agents themselves are created with the generic [`agent` chart](https://github.com/giantswarm/agent), one release per agent. |
