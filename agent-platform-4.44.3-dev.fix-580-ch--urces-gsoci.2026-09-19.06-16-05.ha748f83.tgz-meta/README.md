...
The defaults carry no filter: a release selects releases. agentlab follows a branch of this repository with `agentlab configure --chart-branch <branch>` and installs a checkout's chart directly through `platform.chartPath`; component overrides go into a `platform.valuesFiles` overlay. A dev build's images are published to gsoci like a release's, so the filter is all a consumer of the kagent or Substrate dev channel sets. `make verify-components` asserts the defaults render no filter and that a filter set on one component reaches that component's `OCIRepository` alone.
...
`components.kagent.enabled: true` installs **kagent API v2** — `kagent.dev/v1alpha3`: `AgentTemplate`, `Harness`, `ModelConfig`, `ModelProviderConfig`, `RemoteMCPServer`; agents run as [Agent Substrate](https://github.com/kagent-dev/substrate) actors in gVisor worker pods — from the kagent line Giant Swarm maintains: [giantswarm/kagent-upstream](https://github.com/giantswarm/kagent-upstream), branch `giantswarm`, upstream kagent `main` at a pinned commit plus the carried patches; its `FORK.md` is the ledger of pin, patches and published artifacts. The line publishes from CircleCI, signed with the Giant Swarm identity: two charts, `kagent` and `kagent-crds`, at `oci://gsoci.azurecr.io/giantswarm/kagent/helm` and the multi-arch images `gsoci.azurecr.io/giantswarm/kagent/{controller,ui,golang-adk,claude-harness}` — as releases `vX.Y.Z-gs.N` (`X.Y.Z` the upstream version the pin anticipates, `N` the line's counter) and, per push to the consumed branch, as dev builds. Two roster entries: `components.kagent` and `components.kagent-crds`, the CRDs as their own chart in the kserve-crd shape (templates carrying `helm.sh/resource-policy: keep`, no `crds:` policy). `kagent-crds` follows `components.kagent` unless switched explicitly, the kagent release and the connectivity release `dependsOn` it, and the render refuses kagent on without it.
