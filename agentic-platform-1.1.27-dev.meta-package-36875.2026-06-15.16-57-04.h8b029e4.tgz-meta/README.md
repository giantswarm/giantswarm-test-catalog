...
- **On/off and per-component values are unchanged** — the existing `muster:`,
  ...
  blocks and `*.enabled` toggles still drive each component; their values are
  passed straight through to the rendered release (`HelmRelease.spec.values` /
  Argo `valuesObject`), with the umbrella `global` injected as before.
