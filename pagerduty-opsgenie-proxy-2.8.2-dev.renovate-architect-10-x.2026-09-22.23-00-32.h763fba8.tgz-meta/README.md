...
- `incident.acknowledged`: Acknowledges the corresponding alert in Opsgenie to notify Adidas team who from us took the alert. Acks performed by the proxy's own automation user (`PAGERDUTY_USER`) are skipped, so the automatic re-ack that follows an unacknowledgement does not re-acknowledge the Opsgenie alert.
