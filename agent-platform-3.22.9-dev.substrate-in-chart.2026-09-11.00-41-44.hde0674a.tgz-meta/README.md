...
The connectivity chart renders `kyverno.io` objects: two ClusterPolicies that give the bundled declarative agents their securityContext and the agent-sandbox pod-security ClusterPolicy, and — with Agent Substrate on — the four PolicyExceptions of Substrate's workloads ([Agent Substrate](#agent-substrate)) plus the CNPG ImageVolume exception when pgvector mounts from an image. On a cluster with no Kyverno they would fail the install on an unknown API group, so `kyvernoPolicies.enabled: auto` (the default) renders none of them where `kyverno.io/v1` is not served; `false` forces that, `true` forces them on.
