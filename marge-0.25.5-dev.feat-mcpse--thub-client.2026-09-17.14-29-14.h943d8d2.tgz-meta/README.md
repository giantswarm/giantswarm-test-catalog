...
The `marge` Helm chart in the [giantswarm catalog](https://github.com/giantswarm/giantswarm-catalog) runs `marge serve` over the MCP Streamable HTTP transport behind a `ClusterIP` Service. With `muster.register=true` it also registers the server with [muster](https://github.com/giantswarm/muster), which then runs the GitHub sign-in for it and attaches each person's grant to their calls; the server pod holds no GitHub credential of its own. The registration uses the OAuth client the muster's other GitHub-backed servers share (`muster.github.clientCredentialsSecret`, by default `github-oauth-client` in muster's namespace): muster keeps one client and one grant per issuer, so a person signed in to one GitHub-backed server is signed in to marge, and marge acts with that person's own GitHub rights. The sweep App backs the scheduled runs only. See [helm/marge/README.md](helm/marge/README.md) for every value.
...
muster watches MCPServers in its own namespace only, so `muster.namespace` names it when marge runs elsewhere. The shared client Secret lives in that same namespace, which muster reads without extra RBAC.
