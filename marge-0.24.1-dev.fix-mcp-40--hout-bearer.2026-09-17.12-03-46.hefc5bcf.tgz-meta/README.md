...
helm install marge oci://gsoci.azurecr.io/charts/giantswarm/marge --version 0.9.0 \
  ...
  --set marge.github.app.oauth.existingSecret=marge-github-oauth
```

muster watches MCPServers in its own namespace only, so `muster.namespace` names it when marge runs elsewhere. The OAuth client Secret lives in that same namespace: muster's RBAC lets it read Secrets there and nowhere else. An existing Secret is created there; inline `clientId` and `clientSecret` are written there by the chart.
