...
| `workerpools.ate.dev`, `sandboxconfigs.ate.dev`, `csidriverconfigs.ate.dev` | the **substrate-crds** component chart (the Substrate line, `giantswarm/substrate`) — as templates. Follows `components.kagent`; the kagent release's `WorkerPool` and the substrate release's `SandboxConfig` are its CRs. |
...
Each component that ships a `crds/` dir sets `crds: CreateReplace` on its `HelmRelease` (rendered by the meta-package), so Flux applies and upgrades those CRDs atomically with the app at the same resolved version — Helm on its own never upgrades `crds/`-dir CRDs. The kagent-crds, kserve and cloudnative-pg charts render their CRDs as ordinary templates instead, so their `HelmRelease`s carry no `crds:` policy and Helm applies and upgrades them with every release. All these CRDs carry `helm.sh/resource-policy: keep` (CNPG's through the operator chart's own handling), so they survive a component uninstall (the CRs are never cascade-deleted).
