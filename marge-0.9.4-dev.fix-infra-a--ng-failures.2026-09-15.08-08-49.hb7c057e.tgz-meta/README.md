...
#### CI unavailable (infra)

A check also reports `failure` when the job was stopped, or died before it reached its first step. Nothing was built, tested or scanned, so the check carries no verdict on the code. Two shapes are recognised:

- **Stopped.** The check run concluded `cancelled` or `timed_out`. This needs no message and costs no extra request: a job that was killed scanned nothing.
- **The action could not be fetched.** The runner reports `Failed to download action ...` or `Failed to download archive ...`, typically with an HTTP 429, 502 or 503 while GitHub's own download path is degraded. marge matches those phrases in the check run's output fields and annotations. GitHub records them in the job log and they reach a check run only when the workflow surfaces them there, so this is a best-effort second rule; the conclusion rule above is the unconditional one.

When **every** failing check on a PR is such a check, marge classifies the PR as `CI unavailable (infra)` rather than `Failed`, with a detail naming each check and why it produced no verdict: `CI unavailable (infra) (checks never ran: publish / gitleaks (cancelled before it finished))`. It is counted separately, surfaced under its own section, and kept out of any rescue path -- the fix is to rerun the checks. A PR with a mix of a genuine failure and a stopped job is still reported as `Failed`, and the stopped job is left out of the failure list.

This matters most for a security check. `Failed (security)` is the one category an operator is trained never to wave through, so a gitleaks job that was cancelled before it scanned anything would draw manual attention and discourage the rerun that actually fixes it. A stopped or never-started security check is therefore never reported as a security failure; a scan that ran and found a secret still is.

#### Blocked (CircleCI project setting)

A repository whose `.circleci/config.yml` uses a setup workflow while the project setting that allows it is off gets its pipeline refused by CircleCI:
...
CircleCI reports this in the commit status description and, for the CircleCI GitHub app, in the check run output. No branch fix and no rerun resolves it, so marge classifies such a PR as `Blocked (CircleCI project setting)` and names the setting in the detail: `Blocked (CircleCI project setting) (CircleCI setup workflows not enabled: ci/circleci: CircleCI Pipeline; enable Project settings > Advanced > Dynamic config using setup workflows)`. Somebody has to change the setting once per repository; until then every sweep would otherwise report the PRs as failures and cost an operator a log dive each. Like the other buckets it is counted separately and kept out of the rescue path, and a PR with a genuine failure alongside stays `Failed`.
...
Processes all matching PRs without interactive grouping. The live table shows every PR's outcome, including the failure reason and any ai-rescue marker, followed by a one-line summary; nothing is repeated below the table. With `--no-tui` the results are printed as plain-text groups instead: **Merged**, **Security failures**, **Failed** (PRs that failed, have conflicts, or came from untrusted authors), **Stale** (PRs whose failing checks are already green on their base branch; **Refreshed** once `--refresh-stale` has updated them), **Cancelled** (PRs whose failing CircleCI builds were auto-cancelled by CircleCI; **Retried** once `--retry-cancelled` has retried them), **CI unavailable (Actions budget)** (PRs whose checks never ran because an Actions spending limit was exhausted), **CI unavailable (infra)** (PRs whose failing checks were stopped or died before their first step), **Blocked (CircleCI project setting)** (PRs whose CircleCI pipeline a repository setting refuses), and **Skipped**. Security failures (e.g. govulncheck, Trivy, CodeQL) are separated so they are not mistaken for ordinary CI flakiness; stale, cancelled, budget-blocked, infra and setting-blocked PRs are separated so they are not mistaken for genuine failures (see [Stale failures](#stale-failures-already-fixed-on-the-base-branch), [Cancelled builds](#cancelled-builds-circleci-auto-cancel), [CI unavailable (Actions budget)](#ci-unavailable-actions-budget), [CI unavailable (infra)](#ci-unavailable-infra) and [Blocked (CircleCI project setting)](#blocked-circleci-project-setting)).
