...
| `INCIDENT_TITLE_PATTERN` | Regex to extract installation, cluster ID, and alert name | `(?P<installation>[a-z0-9]+)-(?P<cluster_id>[a-z0-9]{5,})\s*-\s*(?P<alert_name>.+)` |

### Cluster ID Extraction

The service uses regex patterns to extract installation, cluster IDs, and alert names from incident titles. The default pattern matches:
- `grizzly-5e4cr`
- `goggle-abc123`

Customize the pattern via `INCIDENT_TITLE_PATTERN` environment variable.
...
Check the `INCIDENT_TITLE_PATTERN` regex against your incident titles. Enable `LOG_LEVEL=DEBUG` to see extraction attempts.
