...
| `disable_model_serving` | Remove the slice release cluster-manager created and its backend registration. Refused while models are served on the cluster (named) unless `force`, and plainly when the cluster cannot be read as the caller; refused for a release cluster-manager did not create. The same ordered teardown as `delete_node_pool`'s last pool. |
