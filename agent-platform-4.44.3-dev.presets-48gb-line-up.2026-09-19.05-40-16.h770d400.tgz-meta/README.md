...
| `agent-platform-connectivity` | the consumer-side **wiring** the meta-package renders as a child release: the public muster route, the agentgateway data-plane `Gateway` + `AgentgatewayParameters` + `HTTPRoute`s + `BackendTrafficPolicy`s, the `NetworkPolicy`s, the kagent controller `GRPCRoute` with its JWT policy, the klaus-gateway/model-manager/agent-manager routes, the admin-owned kagent catalog (`ModelConfig`s and operator `RemoteMCPServer`s at `kagent.dev/v1alpha3`; the per-agent muster `RemoteMCPServer` is the agent chart's), the `kagent-flux` tenant identity, and the CNPG `Cluster`. Agents themselves are created with the generic [`agent` chart](https://github.com/giantswarm/agent), one release per agent. |

> **CRDs are app-owned.** There is no longer a standalone `agent-platform-crds` bundle chart — each component ships its own CRDs: in its chart's `crds/` dir, upgraded atomically with the app via Flux `CreateReplace` (muster, agentgateway, agent-sandbox), or as templates of its own CRD chart (`kagent-crds` for the kagent line, `kserve-crd`, `kserve-llmisvc-crd`, `cloudnative-pg`). A CR consumer `dependsOn` the component that owns the CRD it needs. See [CRD lifecycle](#crd-lifecycle).
...
- **The wiring chart has no version of its own.** `agent-platform-connectivity` is published off the same tag as this chart, so `components.agent-platform-connectivity` is `releasedWithChart: true`: its `OCIRepository` carries this chart's own exact version (a pre-release kept, build metadata dropped) and the connectivity release rolls exactly when the meta chart rolls — never behind it (a lagging pin refuses the keys the newer meta chart forwards; #431, #441, #339) and never ahead. A `versionRange` on the entry is refused unless it comes with a `semverFilter` (a dev channel) or another `repository` (a chart pushed by hand); a customer BOM does not pin it.
...
Every push to a branch of this repository publishes both charts as **dev builds** — `X.Y.Z-dev.<branch, lowercased to [a-z0-9-]>.<YYYY-MM-DD>.<HH-MM-SS>.h<sha7>`, one version for the two charts of a commit — to `oci://gsoci.azurecr.io/charts/giantswarm/{agent-platform,agent-platform-connectivity}`; the component repositories publish theirs the same way (the kagent line's as `0.11.0-dev.giantswarm.…` under `ghcr.io/giantswarm/kagent`). Two knobs make a consumer follow such a build instead of the releases the defaults select:

- `components.<name>.semverFilter` — a Go regexp rendered as `OCIRepository.spec.ref.semverFilter`, which Flux applies to the registry's tags *before* it evaluates the range. With a prerelease-admitting range (`>=3.0.0-0 <4.0.0-0`: Flux's Masterminds semver never matches a prerelease against a bound without a `-0`, and rejects the RFC's `*-*`) and one branch's filter (`.*-dev\.<sanitized-branch>\..*`) the component follows that branch's newest build and never falls back to a release. An exact pin (`versionRange: "<full dev tag>"`) selects one build without a filter.
- `gitops.self.semverFilter` — the same for the chart's own source under self-management.

The defaults carry no filter: a release selects releases. agentlab follows a branch of this repository with `agentlab configure --chart-branch <branch>` and installs a checkout's chart directly through `platform.chartPath`; component overrides go into a `platform.valuesFiles` overlay. A dev build's images are not copied to gsoci (retagger copies releases): a consumer of the kagent or Substrate dev channel sets `kagent.registry: ghcr.io` and `substrate.image.registry: ghcr.io/giantswarm/substrate` next to the filter. `make verify-components` asserts the defaults render no filter and that a filter set on one component reaches that component's `OCIRepository` alone.
...
| kagent's CRDs | `components.kagent-crds.enabled` | follows `components.kagent` |
| Agent Substrate (the actors' runtime) and its CRDs | `components.substrate.enabled`, `components.substrate-crds.enabled` | follow `components.kagent` |
...
| Model serving on llm-d (presets, discovery ConfigMap, cache, models Gateway) — a feature switch, no chart | `components.modelServing.enabled` | `false` |
...
**The kagent CRDs' storage version (3.x → 4.x).** A cluster that ran kagent 0.10 stores `modelconfigs`, `modelproviderconfigs` and `remotemcpservers.kagent.dev` at `v1alpha2`; the kagent line's `kagent-crds` chart declares `v1alpha3` only, and the apiserver refuses a CRD update that drops a version still in `status.storedVersions` — so `kagent-crds` could never install on an upgraded cluster (giantswarm/agent-platform#396). Whenever `components.kagent-crds` is on the chart runs two hook Jobs as the hook ServiceAccount, in `alpine/k8s`: `<release>-kagent-storage-version-backup` (`pre-install,pre-upgrade`, weight -7) records every object of each of the three CRDs that stores `v1alpha2` — `status` and the server-set metadata stripped — into ConfigMap `kagent-storage-version-migration` in the kagent namespace (`modelconfigs.json`, `modelproviderconfigs.json`, `remotemcpservers.json`, `recorded-at`; replaced whole on every run that records, left alone by a run that finds nothing at `v1alpha2` — a re-run after a failed first upgrade keeps the first run's record for the restore) and then deletes those CRDs, their objects with them, so `kagent-crds` installs them at `v1alpha3` — and, on every run, re-points the three kinds from `kagent.dev/v1alpha2` to `v1alpha3` in every Helm release manifest that still names them (the release Secrets — what `helm mapkubeapis` does for a removed API): Helm reads a release's stored manifest back through the cluster's REST mappings on each upgrade and uninstall, so without this the 3.x connectivity release (its catalog) and the kagent 0.x release (its default `ModelConfig`, its tool-server `RemoteMCPServer`) could never upgrade once `v1alpha2` is gone (`unable to build kubernetes objects from current release manifest: no matches for kind "RemoteMCPServer" in version "kagent.dev/v1alpha2"`); the objects themselves went with the CRDs, and the release's next upgrade creates what its new chart renders and drops the rest; `<release>-kagent-storage-version-restore` (`post-install,post-upgrade`, weight 0) waits for `modelconfigs.kagent.dev` to be Established serving `v1alpha3` (up to 8 minutes) and re-creates every recorded `ModelConfig` that no Helm release owned (no `meta.helm.sh/release-name`) at `kagent.dev/v1alpha3` — name, namespace, labels, annotations and `spec` unchanged; the spec is the same in both versions — leaving one that is already present to its owner, and marks the ConfigMap `restored-at` so it runs once. The chart-rendered `ModelConfig`s come back from their releases (the kagent release's `providers`, the connectivity release's `kagent.modelConfigs[]`), model-manager re-creates the ones it manages; `RemoteMCPServer`s and `ModelProviderConfig`s are recorded, never restored — the 4.x charts render their own. A CRD that is absent or already stores `v1alpha3` (a fresh install, a second run) is left alone, so the hooks are idempotent and a fresh install sees them do nothing. **These two hooks render with the engine off as well** — a cluster's own Flux runs them, and every installation that ran 0.10 needs the step; they are the one exception to "no hook without the engine" below (and the hook identity's network policy comes with them, see [Uninstalling](#uninstalling)). The backup's re-point lists the release Secrets through the apiserver's Table and decodes them one at a time, so its memory is bounded by the largest release, not by the cluster's Helm history (giantswarm/agent-platform#414). [UPGRADE.md](./UPGRADE.md) "3.x → 4.0 (the kagent line)" has the operator's side.
...
A cluster that runs its own Flux — every Giant Swarm management cluster — sets `components.flux.enabled: false` and installs the chart through that Flux. Nothing of the engine reaches the cluster: no CRD, no operator, no `FluxInstance`, no engine hook, no tenant identity, and the platform `HelmRelease`s carry no `serviceAccountName` unless `gitops.serviceAccountName` says so. Self-management is off with the engine (`gitops.self.enabled: auto`) — that Flux holds the chart's `HelmRelease`, as below. The render is the pure app-of-apps render, byte for byte what it was before the engine existed — plus, with kagent on, the two storage-version hooks of the 3.x → 4.x cut-over and their identity (above), which that Flux runs like any Helm hook.
...
spec:
  interval: 10m
  timeout: 12m          # helm-controller's 5m default is too short for the first 4.x reconcile — see below
...
**Set `spec.timeout` to `12m` or more on that `HelmRelease`.** helm-controller's install is the Helm SDK's `--wait`: it returns when every component `HelmRelease` the chart renders is Ready, and the reconcile fails on `spec.timeout` first — whose default is 5 minutes. With kagent on, the first 4.x reconcile installs nine component `HelmRelease`s plus Substrate and kagent in dependency order (the CRD charts and muster, then connectivity, then Substrate's control plane and its worker, then the kagent controller and the managers) — about 5–6 minutes through a cluster's own Flux on the CI kind cluster: `helm install --wait` takes 220 s for the components with cold image pulls, and the platform reached Ready through Flux in 289 s in the green run. A `HelmRelease` that keeps the default fails its first 4.x reconcile on the timeout and is retried by helm-controller. Measured by the ATS own-Flux scenario, [`tests/ats/test_own_flux.py`](tests/ats/test_own_flux.py), which sets `spec.timeout: 12m` (giantswarm/agent-platform#380: CircleCI job 5941 hit the 5-minute default, job 6033 passed with `12m`; the timings are in [tests/ats/README.md](tests/ats/README.md)). Giant Swarm's management-cluster base sets `20m` on its `HelmRelease`. The uninstall through that Flux — the `HelmRelease` deleted, the component releases removed in reverse-dependency waves — takes about 30–105 s (see [Uninstalling](#uninstalling)).
...
**Uninstalling through that Flux.** Delete the agents' `HelmRelease`s first, while the kagent controller still runs (an `AgentTemplate`'s cleanup on Substrate needs it), then the chart's `HelmRelease`: helm-controller uninstalls the meta release, which deletes every platform `HelmRelease` it rendered in one pass, and finalizes them **all at once, concurrently** — helm-controller has no reverse-`dependsOn` uninstall (fluxcd/flux2#1744), and the chart never touches the cluster's Flux, so nothing orders them. That is safe because the CRD charts keep their CRDs: `kagent-crds` and `substrate-crds` render them as templates carrying `helm.sh/resource-policy: keep` (the kagent line from giantswarm/kagent-upstream#10, the Substrate line from `v0.0.27-gs.4`, giantswarm/agent-platform#385), so a release whose objects are their CRs can always delete them, whatever order the releases go in — before the Substrate line carried the policy, `substrate-crds` could go before `substrate`, whose uninstall then failed for good on its `SandboxConfig` (`failed to delete release: substrate`: Helm cannot delete an object whose kind is gone) and was retried until the meta `HelmRelease`'s finalizer timed out (measured on the ATS own-Flux scenario). What stays: the eight kept CRDs (`kagent.dev`, `ate.dev`), the kagent namespace with the agents' `AgentTemplate`s and `RemoteMCPServer`s, and Substrate's leftovers in `ate-system` (above); no `SandboxConfig` or `WorkerPool` outlives the release that owned it, and one created by hand survives until its owner deletes it. To remove the CRDs as well, `kubectl delete crd` them explicitly — with the objects behind them. On an installation whose Substrate line predates `v0.0.27-gs.4` (the range's floor names it), delete the component `HelmRelease`s in reverse dependency order by hand before the chart's — `kagent`, then `substrate`, then the CRD charts — or the `substrate` release's uninstall fails as described. The ATS own-Flux scenario's way back asserts the concurrent uninstall on every release (`tests/ats/test_own_flux.py`).
...
Uninstalling the platform uninstalls its engine and everything the engine held, in order. Helm deletes a release's objects in one pass, so on its own `helm uninstall --wait` would remove the operator together with the `FluxInstance` whose finalizer it processes and hang for its timeout (measured). The chart therefore ships pre-delete hook Jobs — restricted pods running one plain `kubectl` command in `registry.k8s.io/kubectl` (`gitops.hooks.image`) or a small shell script in `alpine/k8s` (`gitops.hooks.helmImage`), as a hook ServiceAccount bound to `cluster-admin` at weight -10, policed by a network policy of the same weight and events (`<release>-hooks`, egress to the apiserver only, in the flavour `networkPolicy.flavor` resolves to; a default-deny cluster admits nothing else, giantswarm/agent-platform#413) — that stop a self-management resumer still running (weight -6) and suspend the chart's own `HelmRelease` so it is deleted without uninstalling the release and cannot re-create the platform `HelmRelease`s mid-teardown, removing the hook-written values Secret with it (weight -5; these two run in `alpine/k8s`, `gitops.hooks.helmImage`, as the namespaced identity `<release>-self`, and tolerate a missing `HelmRelease`), delete the platform `HelmRelease`s by name in **reverse dependency order** and wait (weight 0; waves computed from the roster's `dependsOn`, each wave's releases uninstalling concurrently, the next wave starting once helm-controller has removed the last of them — so a CRD chart's release, `kagent-crds` or `substrate-crds`, goes only after every release whose objects are its CRs: a Helm uninstall cannot delete an object whose kind is already gone, and helm-controller would retry it until the hook times out), then delete the `FluxInstance` and wait (weight 5). The operator then uninstalls Flux **including the Flux CRDs**, and every `HelmRelease` object in the cluster goes with them: the agents' too, while their `AgentTemplate` and `RemoteMCPServer` objects and Helm storage stay behind, orphaned — the kagent namespace is kept (`helm.sh/resource-policy: keep` on the connectivity release's `Namespace`) and so are the kagent CRDs (the `kagent-crds` release is uninstalled with the others; the line's CRD templates carry `keep`), so a reinstall finds the agents where it left them; the three `ate.dev` CRDs are kept the same way (the Substrate line's templates carry `keep` from `v0.0.27-gs.4` on), while their objects go with the releases that own them — the `substrate` release's `SandboxConfig`, the kagent release's `WorkerPool`. What Substrate leaves behind is different, and a reinstall relies on it: the `ate-system` pools, ate-api-server's authentication config and the bundled Postgres's claim survive (no release owned them), and so do the `podcertificate-controller-system` namespace with its two CA pools — the Substrate line's chart keeps the namespace (`helm.sh/resource-policy: keep`, since 0.0.27-gs.5) — and the signers' cluster-scoped `ClusterTrustBundle`s. The next install finds the pools and keeps them, the podcertificate-controller signs from the roots the bundles already carry, and the bootstrap hook republishes both bundles from the pools before any Substrate pod starts, so a bundle whose roots are not its pool's (a pool rotated by hand; a clean slate that removed the namespace but not the bundles) never reaches a pod — every Substrate client reads its trust anchor once, when it starts (giantswarm/agent-platform#384; the ATS own-Flux scenario reinstalls onto the cluster the smoke uninstalled from with all of it in place). A clean slate is `kubectl delete namespace ate-system podcertificate-controller-system` plus the two `podcert.ate.dev` `ClusterTrustBundle`s. The uninstall returns in about 31 s on the CI kind cluster with kagent, Substrate, agent-manager and two agents on (the five waves; 12–16 s without kagent); the four `fluxcd.controlplane.io` CRDs remain (Helm never deletes `crds/`), and a reinstall is clean. "Keep my agents running" is not something an uninstall can offer — the way to keep agents is not to uninstall. A failed hook aborts the uninstall before anything is deleted (the release is left `uninstalling`, the platform untouched); `helm uninstall --no-hooks` skips the ordering and is only safe once the `FluxInstance` is already gone. With the engine off (a cluster's own Flux) none of this applies: that Flux finalizes the platform `HelmRelease`s, the chart renders no hook and never touches it — see "Clusters that run Flux" for the uninstall there.
...
| `components.kagent.enabled` | `false` | Install the kagent line (kagent API v2): the `kagent` and `kagent-crds` components. See [The kagent line](#the-kagent-line). |
...
model-manager starts with no backend and the backends are registered at runtime (the `add_backend` tool, the portal's Serving page, cluster-manager for a GPU node pool — model-manager's [docs/backends.md](https://github.com/giantswarm/model-manager/blob/main/docs/backends.md)), as labelled ConfigMaps the chart never sees, so its egress policy — rendered from the static inputs above — does not open them. `modelManager.networkPolicy.registeredBackends` lists the destinations the installation allows for them, one entry per backend: `{cidr, port}` in both flavors, `{fqdn, port}` under cilium (a name for an in-cluster Service — `ollama.models.svc.cluster.local` on `11434` — opened through the DNS proxy rule plus the cluster entity, as the static hostname rule; Cilium's CIDR rules match neither pods nor nodes, so a `cidr` there is a destination outside the cluster). Each entry is one more egress rule on its port next to the static-backend rules; empty, the default, renders nothing. The guards fail the render on a `cidr` that is not an IPv4 CIDR, a missing port, both or neither destination, and an `fqdn` under the kubernetes flavor with policies on (giantswarm/agent-platform#478).
...
`components.kagent.enabled: true` installs **kagent API v2** — `kagent.dev/v1alpha3`: `AgentTemplate`, `Harness`, `ModelConfig`, `ModelProviderConfig`, `RemoteMCPServer`; agents run as [Agent Substrate](https://github.com/kagent-dev/substrate) actors in gVisor worker pods — from the kagent line Giant Swarm maintains: [giantswarm/kagent-upstream](https://github.com/giantswarm/kagent-upstream), branch `giantswarm`, upstream kagent `main` at a pinned commit plus the carried patches; its `FORK.md` is the ledger of pin, patches and published artifacts. The line publishes two charts, `kagent` and `kagent-crds`, at `oci://ghcr.io/giantswarm/kagent/helm` and the multi-arch images `ghcr.io/giantswarm/kagent/{controller,ui,golang-adk,claude-harness}` — as releases `vX.Y.Z-gs.N` (`X.Y.Z` the upstream version the pin anticipates, `N` the line's counter) and, per push to the consumed branch, as dev builds; retagger copies every release's images to `gsoci.azurecr.io/giantswarm/kagent/<image>`, the same path under the org's registry with the digests preserved (giantswarm/retagger#1229), which the defaults pull from. Two roster entries: `components.kagent` and `components.kagent-crds`, the CRDs as their own chart in the kserve-crd shape (templates carrying `helm.sh/resource-policy: keep`, no `crds:` policy). `kagent-crds` follows `components.kagent` unless switched explicitly, the kagent release and the connectivity release `dependsOn` it, and the render refuses kagent on without it.

**One build of the line.** Two values name it and move together: `components.kagent.versionRange` = `components.kagent-crds.versionRange` (`>=0.11.0-gs.6 <0.11.1-0`, the line's releases of upstream 0.11.0 from the first chart that carries its own image stamps: Flux's Masterminds semver compares prerelease identifiers left to right, so the line's `0.11.0-dev.giantswarm.…` builds fall below the `gs.1` floor and a later `-gs.N` is admitted; the `-0` on the ceiling lets prereleases through at all, and the ceiling holds the patch because a prerelease is confined to no patch tuple; a bare `0.11.0` would be admitted too, should upstream's release ever appear at this OCI path — by design the switch to an upstream release is a range change). The build's image references travel **inside the chart**: the line's tag workflow stamps `tag` (the controller and UI image tag = the chart version), `controller.agentImage.digest` (the Go ADK image the platform `Harness` runs, **by digest** — the CRD accepts nothing else), `substrateWorkerPool.workerImage` (the gVisor worker of the Substrate version the build was made against) and a ConfigMap `kagent-images` of those digests into every published chart. This chart forwards none of them by default — `kagent.tag` is gone, `kagent.substrateWorkerPool.workerImage` and `kagent.harness.image` are empty and travel only when an installation sets them (`components.kagent.omitEmptyKeys`); an empty value would replace a stamp.

**Re-pinning the kagent line.** A new release of the line is a values change here, not a Flux roll: take the release's row from [`builds.md`](https://github.com/giantswarm/kagent-upstream/blob/ledger/builds.md) on the fork's `ledger` branch (one row per published build, every image's index digest), move the two `versionRange`s' floor to its version (and the base, when the upstream version moved) and `components.substrate`/`substrate-crds` to the Substrate version its stamped `workerImage` names — the tag, the Go ADK digest and atelet's pinned set follow from the chart. `make verify-meta` asserts nothing here names a build (no `tag`, `workerImage` or Harness digest forwarded), that the Harness policy is forwarded, and that the range admits only the line's releases from the floor; `make verify-components-charts` renders the forwarded `kagent` and `kagent-crds` blocks — like every component's — against the charts the ranges and the BOM pins resolve to. The customer BOM pins `kagent` and `kagent-crds` to one release.

**What the chart forwards.** The `kagent:` block is upstream's chart shape (keys at the root): `registry: gsoci.azurecr.io` (the repositories and the stamped digests stand as the chart publishes them; the dev channel sets `ghcr.io`), the fork's image names, `controller.substrate.*` (the controller talks to Substrate's `ate-api` and routes through `atenet` in `ate-system`), `substrateWorkerPool` (the `WorkerPool` `kagent-default` the platform Harness runs on: `replicas` bound the concurrently active agents, `template` carries resources and node placement; `workerImage` is the chart's stamp unless set), `harness` (the platform `Harness` `kagent` the chart renders — `create: true`, the snapshot location, `KAGENT_PROPAGATE_TOKEN`, the admission label `agent-platform.giantswarm.io/harness: kagent` and the chart's own `kagent.dev/harness` key blanked — an empty value, which the line's Harness template (`0.11.0-gs.9`+) drops from the selector; a null would delete the key on coalesce only where it reaches the chart, and the merge patch of a kagent HelmRelease that pre-existed the cut-over removes a null instead of storing it (giantswarm/agent-platform#418); the image is the chart's stamped Go ADK digest unless `kagent.harness.image` is set), pgvector on the bundled Postgres (the line's memory API needs it; with the platform CNPG cluster set `postgres.vector.enabled`), `kmcp` / `kagent-tools` / `grafana-mcp` off. The connectivity chart's keys — `controllerRoute`, `uiRoute`, `modelConfigs`, `remoteMcpServers`, `serviceMonitor`, `oauth2ProxyIngress`, `fluxServiceAccountName` — stay in the block and are dropped on the way to the kagent release (`components.kagent.omitKeys`): the kagent chart has no schema and would carry them on silently. `kagent-crds:` keeps the chart's `kmcp` and `substrate` CRD subcharts off — muster fills kmcp's role, Substrate's CRDs come with the `substrate-crds` component.

**Drift detection.** The kagent and muster HelmReleases carry `spec.driftDetection.mode: enabled` (`components.<name>.driftDetection`; a passthrough every component entry takes, set for these two only): on every reconcile — `gitops.interval` — helm-controller compares the release manifest with the cluster and re-applies what differs or is missing, as a server-side apply without a Helm revision. So the platform Harness — deleted by the 4.8.0 upgrade of a consumer whose pinned connectivity chart skipped 4.7.19's `keep`, and never recreated by a release without drift detection (a plain reconcile of an unchanged release reports "in-sync") — is back within one interval and every template it admits returns to Ready (giantswarm/agent-platform#409; the ATS smoke deletes it and sees it back). muster carries it for the same reason one step down: its chart's `CiliumNetworkPolicy` kept the pre-rename namespace `agentic-platform` in its egress selectors on a management cluster that came through the rename, while the release manifest named `agent-platform`. Helm's three-way merge patches only what changed between two release manifests, so the live object stayed drifted across every upgrade; muster's egress to Valkey was denied, its OAuth server never started and `/.well-known/oauth-protected-resource` answered 503 for hours after a routine pod restart (giantswarm/agent-platform#287; the ATS smoke edits an object of the release and sees it reverted). A hand edit to an object of either release is reverted the same way; an object opts out with the annotation `helm.toolkit.fluxcd.io/driftDetection: disabled`. Turning it on for a third release is a decision per release: its objects must tolerate the re-apply, and a field another controller owns needs an `ignore` rule. On muster that is `spec.replicas` of the Deployment under `muster.autoscaling.enabled: true` — the chart renders the field whether or not the HPA is on, so an installation that turns the HPA on must add `components.muster.driftDetection.ignore` for it.

**Prerequisites.** Kubernetes 1.35 with the three feature gates and Agent Substrate — the runtime the chart ships next to kagent; [Agent Substrate](#agent-substrate) has the details. Upgrading a 3.x installation: [UPGRADE.md](./UPGRADE.md).

### Agent Substrate

kagent API v2 has no runtime without [Agent Substrate](https://github.com/kagent-dev/substrate): every agent runs as a Substrate *actor* in a gVisor *worker* pod, a `Harness` names a `WorkerPool` and a snapshot location, and the kagent controller drives it all through Substrate's `ate-api`. The chart ships Substrate as two components in the `kagent-crds` shape — `components.substrate-crds` (the three `ate.dev` CRDs as templates) and `components.substrate` (the control plane in `ate-system`: `ate-api-server`, `ate-controller`, the `atenet` router, egress gateway and dns, the per-node `atelet` DaemonSet; the `podcertificate-controller` in `podcertificate-controller-system`) — from the [Giant Swarm Substrate line](https://github.com/giantswarm/substrate), branch `giantswarm`: the upstream release kagent's `go.mod` pins plus the carried patches; its `FORK.md` is the ledger. Both follow `components.kagent` unless switched explicitly, and the render refuses kagent on without them. Both land in `ate-system` (`components.<name>.targetNamespace` — the chart's Roles and Service names are fixed to it), and both carry **one version pin**: the same range in the two `versionRange`s (`>=0.0.27-gs.5 <0.0.28-0`, the kagent entry's shape — the line's releases of upstream 0.0.27 from `v0.0.27-gs.5` on: the egress gateway's frontend policy for its agentgateway dataplane, the connection string from a Secret, atelet scheduling, `helm.sh/resource-policy: keep` on the three CRD templates, so no concurrent uninstall fails on a vanished kind — giantswarm/agent-platform#385 — and, from `v0.0.27-gs.4`, a golden boot whose image the registry refuses failing with the reason instead of pending forever — giantswarm/substrate#14, and the `podcertificate-controller-system` namespace kept across an uninstall so a reinstall keeps its trust chain (gs.5, giantswarm/agent-platform#384)), whose floor is the version `kagent.substrateWorkerPool.workerImage` names — the control plane and the workers are one Substrate version. A re-pin of Substrate moves the three together, with the kagent pin (`make verify-components` holds them to one).

**Why roster components and not the kagent charts' `substrate` subchart switches:** each Substrate release has its own `HelmRelease` status on the installation, the order is explicit (`substrate-crds` → `agent-platform-connectivity` → `substrate` → `kagent`), the repository is a value (`components.substrate.repository`, a mirror for a proxied installation, with `substrate.image.registry` for the images), and Substrate's namespace is not kagent's.

**Bootstrap.** The substrate chart mounts material it does not render: the CA pools `service-dns-ca-pool` and `pod-identity-ca-pool` the podcertificate-controller signs from, the JWT authority pool `actor-id-jwt-pool` and the CA pool `actor-id-ca-pool` ate-api-server mints actor identities from, the trust anchor `actor-id-ca-certs` derived from the latter, and the ConfigMap `ate-api-authentication` naming the apiserver's ServiceAccount issuer — verbatim as the apiserver's OpenID discovery document spells it (a kubeadm cluster issues `https://kubernetes.default.svc.cluster.local`; a CAPA cluster with IRSA its public issuer; ate-api-server compares the token's `iss` literally). Upstream does this with four `kubectl-ate admin` invocations and a shell step; here the connectivity release's `pre-install,pre-upgrade` hook Job `<release>-substrate-bootstrap` mints the same key material (`openssl` in an init container: an ED25519 root, 365 days, `IsCA` + `keyCertSign` per CA pool; an ECDSA P-256 ES256 authority — written in the pools' wire format) and creates what is missing. A pool that exists is **never touched** — a regenerated root would orphan every certificate and token issued from it while the pods holding them keep running — so every upgrade re-runs the hook as a no-op that logs `present`; the trust anchors follow the pools, applied server-side on every run: `actor-id-ca-certs` from `actor-id-ca-pool`, and the podcert signers' `ClusterTrustBundle`s (`servicedns.podcert.ate.dev:identity:primary-bundle`, `podidentity.podcert.ate.dev:identity:primary-bundle`) from the two podcertificate pools — the podcertificate-controller publishes and refreshes those itself, but a pod projects the bundle that exists when it starts and reads it once, so the hook publishes them first, before the `substrate` release installs (giantswarm/agent-platform#384); the authentication ConfigMap is kept and its issuer compared. The hook runs as `<release>-hooks` (a ClusterRole on secrets, configmaps and namespaces — get, list, watch, create, patch; it creates the two namespaces bare when missing, the substrate release adopts them) for its lifetime only. Because Substrate's pods start against what the hook mints, and kagent's against the CNPG connection Secret the same release derives, both `dependsOn` the connectivity release and install with a real readiness wait: the install and the 3.x → 4.0 upgrade order is `substrate-crds`, `kagent-crds` → `agent-platform-connectivity` → `substrate` → `kagent` → the managers.

**The control-plane database.** `substrate.postgres.enabled` is `auto`: with the platform's CNPG Cluster on (`postgres.enabled`) Substrate's database is the CNPG `Database` `postgres.databases.substrate` on that Cluster (spec.name `substrate`, owned by the application role, `retain`), ate-api-server reads its DSN from the derived Secret `<postgres.clusterName>-substrate-app` in `ate-system` (the meta chart derives `substrate.postgres.connectionStringSecretRef` from those values; the connectivity release's `post-install,post-upgrade` hook `<release>-postgres-databases` waits for CNPG's `<clusterName>-app` and copies it with `dbname`, `uri`, `jdbc-uri` and `pgpass` rewritten — CNPG mints that Secret for the initdb database only, a `Database` mints none), and the chart's bundled single-instance StatefulSet stays off; without the Cluster the bundled StatefulSet runs (a lab, a quick start), as kagent's own bundled Postgres does. `true` forces the bundled one, `false` refuses it, `substrate.postgres.connectionString` names an external database; a Substrate with none of the three fails the render. `postgres.databases` is a map (one `Database` + one derived Secret per entry, copied into every `secretNamespaces` entry), so an installation adds its own databases next to the shipped ones and the kagent database of the cut-over joins the same map.

**The snapshot store.** `kagent.harness.snapshotLocation` — the platform Harness's `snapshotPolicy.location`, an object-store URL such as `s3://<bucket>/<prefix>` — is **required whenever kagent is on** and has no default: the installation names its store, or lets the chart provision it. On CAPA that is an S3 bucket reached through the standard AWS credential chain of `atelet` and `ate-api-server` (IRSA): **`kagent.harness.snapshotStore.crossplane`** renders it the way `postgres.backup.crossplane` renders the kagent-pg backup bucket — the connectivity chart renders the Crossplane `Bucket` (a lifecycle expiration, `aws.lifecycleDays`, 30 by default: an actor whose snapshot expired boots cold from the golden image again; a public-access block; a TLS-only policy; never deleted by Crossplane and kept by Helm) and an IAM `Role` (`aws.roleName`, the bucket name by default) with an inline list/get/put/delete policy on the bucket, trusted through the installation's IRSA OIDC provider (`aws.accountId`, `aws.oidcProvider`) for `system:serviceaccount:ate-system:atelet` and `…:ate-api-server`; the meta chart derives `kagent.harness.snapshotLocation` as `s3://<aws.bucketName>/<prefix>` (`prefix` `kagent` by default; an explicit location that disagrees fails the render naming both keys) and forwards the role's ARN as the `eks.amazonaws.com/role-arn` annotation of the substrate release's `atelet` and `ate-api-server` ServiceAccounts (`substrate.atelet.serviceAccount.annotations`, `substrate.ateApiServer.serviceAccount.annotations` — Substrate `0.0.27-gs.8`+; an installation's own annotations stay next to it). The block is the meta chart's and the connectivity chart's, never forwarded to the kagent chart. Off by default; needs the upbound AWS provider and a `ProviderConfig` on the cluster (`providerConfigRef`, `region`, `tags`, `observeOnly` as for postgres). Without it the installation brings its own bucket and role and names the location by hand. On CAPZ the store is Azure Blob, which Substrate does not speak: **`provider: capz`** renders the storage `Account` (`capz.storageAccountName`; TLS-only, no public blob access, seven-day soft delete; never deleted by Crossplane, kept by Helm), the blob `Container` (`capz.containerName`) with a lifecycle `ManagementPolicy` (`capz.lifecycleDays`, 30), a `UserAssignedIdentity` with a `FederatedIdentityCredential` against the cluster's OIDC issuer (`capz.workloadIdentity.oidcIssuerUrl`) for the s3proxy ServiceAccount and a `RoleAssignment` (Storage Blob Data Contributor, scoped to the container — provider-kubernetes bridges the identity's generated ids into it and into the pods' Secret, `capz.workloadIdentity.providerKubernetes`), and the **s3proxy façade** (`kagent.harness.snapshotStore.s3proxy`, on with the provider): a stateless Deployment in the release namespace — the gsoci mirror of [gaul/s3proxy](https://github.com/gaul/s3proxy) (`s3proxy.image`), two replicas, a PodDisruptionBudget, a network policy admitting only atelet and ate-api-server — that speaks S3 to Substrate (`aws-v4`, a per-installation key pair generated once and kept: the Secret `substrate-s3proxy` in the release namespace and in `ate-system`) and `azureblob` to the account as the identity (`DefaultAzureCredential`; the pods carry `azure.workload.identity/use`). The meta chart derives `snapshotLocation` as `s3://<capz.containerName>/<prefix>` and hands the substrate release `AWS_ENDPOINT_URL=http://substrate-s3proxy.<namespace>.svc:80`, `AWS_S3_USE_PATH_STYLE`, `AWS_REGION` and the key pair through `substrate.atelet.extraEnv` and `substrate.ateApiServer.extraEnv` (an installation's own entries stay; one of those names fails the render; `substrate.rustfs` must be off). The façade also runs **alone** (`s3proxy.enabled: true`, the Crossplane block off) in front of an account provisioned by hand — or a lab's Azurite — named in `s3proxy.azure.{endpoint,account,container}` and reached with an account key (`s3proxy.azure.accountKeySecretRef`). Every snapshot byte crosses the façade's JVM once (`s3proxy.resources`, 1 GiB of memory by default; the container also declares `ephemeral-storage` — 256 MiB requested, 1 GiB limit — and its two emptyDirs take that limit as their `sizeLimit`, both required by the render and by Kyverno's `require-emptydir-requests-and-limits`); measured in agentlab against Azurite: one 2 GiB object, in-cluster aws-cli: PUT 9.8 s (209 MB/s), GET 7.4 s (276 MB/s) through the façade into Azurite, against PUT 5.8 s (356 MB/s), GET 3.0 s (684 MB/s) on the bundled RustFS — the façade's hop costs about 1.7× on writes and 2.5× on reads in the lab; an actor's golden snapshot restores from the store in ~0.24 s (manifest 11 ms, download 37 ms, restore 190 ms). An S3-compatible store elsewhere takes its endpoint and credentials in `substrate.atelet.extraEnv` and `substrate.ateApiServer.extraEnv` (`AWS_ENDPOINT_URL`, `AWS_S3_USE_PATH_STYLE`, `AWS_REGION`, the keys from a Secret; Substrate `0.0.27-gs.8`+ for ate-api-server); `substrate.atelet.storageBackend` selects `s3` or `gcs`. A lab runs the chart's bundled in-cluster store (`substrate.rustfs.enabled: true`, `s3://ate-snapshots/<prefix>`).

**Scheduling.** `atelet` runs on every node the DaemonSet controller schedules to — on a cluster whose control-plane nodes carry the `node-role.kubernetes.io/control-plane` taint, the worker nodes; `substrate.atelet.{nodeSelector,tolerations,affinity}` pin it to a node pool or admit a tainted one. The WorkerPool is pinned to **one CPU feature set** — one vendor and generation, not just one architecture (`kagent.substrateWorkerPool.template.nodeSelector`: `kubernetes.io/arch: amd64`, and on CAPA the vendor `karpenter.k8s.aws/instance-cpu-manufacturer: amd`, because Karpenter consolidation may replace a node with another vendor's at any time; the CPU generation `karpenter.k8s.aws/instance-generation: "6"` — quoted, a label value is a string — because a newer generation's snapshot does not restore on an older one of the same vendor (gazelle 2026-09-14, giantswarm/agent-platform#457); pick a generation that is one CPU model across its families, for AMD on AWS 6 or 7, never 5; on the other providers the node pool or `node.kubernetes.io/instance-type`): an actor's golden snapshot is a gVisor checkpoint that restores only on a host whose CPU offers every feature it recorded, and a worker on a CPU that lacks one fails every restore placed on it (`incompatible FeatureSet: missing features: …` in the worker's log, the turn `request timed out`; giantswarm/agent-platform#429). Its replicas and the rest of its pod template are the capacity knobs; a `PodDisruptionBudget` on its workers (`kagent.substrateWorkerPool.podDisruptionBudget`, `maxUnavailable: 1`, on by default — the connectivity chart's object in the kagent namespace) makes a voluntary drain move one worker at a time, and the template's Karpenter knobs (`karpenter.sh/do-not-disrupt`, `karpenter.sh/capacity-type: on-demand`, both unset by default) and the render guard on what `WorkerPool.spec.template` would prune silently (a spread constraint, until the Substrate line carries it) are in the chart README's "The one pool is the failure domain" (giantswarm/agent-platform#472).

**Kyverno.** On a cluster that enforces the restricted Pod Security Standard through Kyverno, the connectivity chart ships one `PolicyException` per Substrate workload — `substrate-atelet` (privileged, hostPorts 8085/9090, hostPath mounts), `substrate-workers` (the worker pods of every WorkerPool: root, the gVisor capability set, AppArmor and seccomp `Unconfined`, a hostPath with `HostToContainer` propagation), `substrate-control-plane` (the Deployments in `ate-system` declare no securityContext; matched by workload name, since `ate-api-server` and `ate-controller` carry no labels of their own), `substrate-podcertificate-controller` — each naming exactly the rules its pod spec violates, with their `autogen-` copies; `kyvernoPolicies.rules` maps each rule to the cluster's ClusterPolicy (the upstream kyverno-policies names by default). `make verify-kyverno` computes the violations from the rendered pod specs and holds the exceptions to them. [docs/substrate-security.md](./docs/substrate-security.md) is the write-up for installation owners and security reviewers.

**Network policies** (`networkPolicy.flavor`, both flavours): Substrate's hops — `atelet` ↔ `ate-api`, the kagent controller → `ate-api` and the `atenet` router, the router → a worker's tunnel on 443, `ate-api` → `atelet`'s hostPort 8085 (the egress rule upstream lacks), the workers → the egress gateway and the dns — and the actors' destinations on the **egress gateway** (`atenet-egress`), where an actor's outbound connections leave: muster, the kagent controller, the LLM provider (or the agentgateway LLM listener), DNS. The worker pods reach only the egress gateway, the dns and the cluster DNS. The kubernetes flavour renders the ingress policies. `make verify-kagent-netpol` asserts the render; the runtime proof is an installation with Cilium enforcing — agentlab renders no network policies.

**The chart's third-party images.** Next to the line's own components the substrate chart runs images that are not its own — the bundled control-plane database (`postgres`), the bundled snapshot store and its bucket-init Job (`rustfs`, `awsCli`), the agentgateway build of the atenet router and egress (`agentgateway`) — from an `images:` map whose defaults are Docker Hub short names (`postgres:18-alpine@…`, `rustfs/rustfs:…`, `amazon/aws-cli:…` — which registry a short name means is the node runtime's call: containerd says Docker Hub, CRI-O in short-name enforcing mode refuses it) and the agentgateway line's ghcr build. `substrate.images` pins each to its `gsoci.azurecr.io/giantswarm/…` copy at the same digest (retagger's mirrors of `library/postgres`, `rustfs/rustfs` and the agentgateway line's release tags), so the fleet pulls them from the one registry `make verify-images` holds every default to (giantswarm/agent-platform#575) — with one exception the block names on purpose: `images.awsCli` stays the chart's own short name, because the rustfs-bucket-init Job is a release resource whose pod template is immutable and a changed image fails the upgrade of every installation running the bundled store until someone deletes the completed Job; it moves with the Substrate release whose Job can be recreated (its gsoci copy, `giantswarm/aws-cli:2.17.0`, exists). The pins are copies of another chart's defaults, so they hold for the Substrate release `components.substrate.versionRange` confines and **a Substrate re-pin moves them with it**: `make verify-substrate-images` pulls every release the range admits, requires each pin to carry the release default's digest (or tag — the agentgateway build the egress config is written for) under gsoci and to name a key the release knows, and renders the release with the forwarded block, the bundled store and database on, refusing any third-party image left off gsoci. The line's own defaults are the durable fix (its `FORK.md`); until its release carries them, this block is the fleet's.

**Prerequisites.** Kubernetes **1.35** (Substrate's `PodCertificateRequest` is `certificates.k8s.io/v1beta1`, which 1.34 serves only as `v1alpha1`) with the feature gates `ClusterTrustBundle`, `ClusterTrustBundleProjection` and `PodCertificateRequest` on kube-apiserver, kube-controller-manager **and every kubelet**; enabling them rolls the control plane and every node. Where the render is live (the Helm CLI, `--dry-run=server`, helm-controller) it refuses a cluster that does not serve `certificates.k8s.io/v1beta1/PodCertificateRequest`, naming the gates — the kubelet gates cannot be seen from the apiserver and the message says so; `helm template` and CI, which see no cluster, are never refused. On a Giant Swarm cluster the cluster chart's `internal.advancedConfiguration.{controlPlane.apiServer,controlPlane.controllerManager,kubelet}.featureGates` bridges until giantswarm/cluster#1005 makes the gates the default; each list replaces the chart's default list, so repeat the defaults next to the three gates. The line's images come from `gsoci.azurecr.io/giantswarm/substrate` (`substrate.image.registry`; retagger's copy of every release under its upstream path, giantswarm/retagger#1229 — the control plane and the derived worker image follow the one value). A proxied installation mirrors `oci://ghcr.io/giantswarm/substrate/helm` (`components.substrate.repository`), that registry, the chart's third-party images (`substrate.images`, gsoci copies by default — above), the two hook images (`hooks.kubectlImage`, `hooks.opensslImage`) and the gVisor release asset the chart's `SandboxConfig gvisor-default` names (`gs://gvisor/releases/…`, fetched by `atelet` from `storage.googleapis.com` — override `spec.assets` on the SandboxConfig for a mirror). Upgrading a 3.x installation: [UPGRADE.md](./UPGRADE.md).

### Backstage, mcp-kubernetes, CloudNativePG and KServe

The components the [agent-platform-standalone](https://github.com/giantswarm/agent-platform-standalone) umbrella carried as Helm dependencies on top of this roster, so that a cluster with none of them can turn them on from the same `components:` map. All six are **off by default**: a management cluster runs each of them as its own app and keeps them off, and the fleet render is unchanged. Each is rendered by the same loop as one `OCIRepository` + `HelmRelease`; the values blocks (`backstage:`, `mcp-kubernetes:`, `cloudnative-pg:`, `kserve-llmisvc-crd:`, `kserve-llmisvc-resources:`, `kserve-runtime-configs:`) carry the standalone's defaults and are forwarded verbatim, with `global` injected (all six charts accept it).
...
**Turning them on.** `components.<name>.enabled: true`. Backstage and mcp-kubernetes also need `global.domain` and `global.identity` (`issuerUrl`, `clientId`, `existingSecret` — the platform credentials Secret, with the keys `dex-client-secret` and, for Backstage, `backstage-session-secret`): the same quick-start inputs muster takes. The mcp-kubernetes chart fails its render without them, by design; the Backstage values mount that Secret by name. `kserve-llmisvc-resources` needs cert-manager on the cluster and renders the control plane's shared objects itself — the `inferenceservice-config` ConfigMap, the self-signed `Issuer`, the default `ClusterStorageContainer` (`kserve.createSharedResources: true`); it is the platform's one KServe controller. On a cluster without Cilium set `mcp-kubernetes.ciliumNetworkPolicy.enabled: false` (see [Prerequisites](#prerequisites)).

**Order.** `dependsOn` replaces the two-phase first-install guard the standalone needed: `kserve-llmisvc-crd` is Established before the llm-d controller and the well-known configs; the operator and the control plane come before their CR consumers — `agent-platform-connectivity` (the CNPG `Cluster` under `postgres.enabled`) `dependsOn` `cloudnative-pg`, `model-manager` (which composes the `LLMInferenceService`s) `dependsOn` `kserve-llmisvc-resources`, `backstage` `dependsOn` `cloudnative-pg` (its database is a CNPG `Cluster` when `backstage.database.engine: postgresql`; the chart's default is sqlite). A reference to a toggled-off component is dropped at render time, as everywhere, so a management cluster that provides the operator and KServe as its own apps sees no change.

**CRDs.** None of the six ships a `crds/` dir. `kserve-llmisvc-crd` and `cloudnative-pg` render their CRDs as ordinary templates — Helm applies and upgrades them with every release, the KServe ones annotated `helm.sh/resource-policy: keep` — so their `HelmRelease`s carry no `crds:` policy; see [CRD lifecycle](#crd-lifecycle). The LLMInferenceService CRDs are their own component (`kserve-llmisvc-crd`): the standalone had to leave that 4.5 MB chart a hand-installed prerequisite because it did not fit next to everything else in one Helm release Secret; as its own release it does.

**Wiring.** The standalone rendered by hand what makes these a platform: the Backstage app-config ConfigMap (`agent-platform-backstage-app-config`, which the `backstage:` block mounts) and `HTTPRoute`, the mcp-kubernetes `MCPServer` registration with muster, the model serving presets and their discovery ConfigMap. That wiring lives in the connectivity chart, gated on these same toggles — see [Turning on the standalone's extras](#turning-on-the-standalones-extras). The six values blocks travel to the connectivity release with the rest of the tree (nothing of theirs is held back; `components.agent-platform-connectivity.omitKeys` stays the tool for a NEW top-level key the live connectivity chart does not declare yet — the new `modelServing:` block is a switch's block and travels only while `components.modelServing` is on, which closes that window without a follow-up release), and the connectivity release `dependsOn` `muster` as well, for the `MCPServer` CRD.
...
helm template r helm/agent-platform -f helm/agent-platform/ci/ci-values.yaml \
  ...
  --set components.cloudnative-pg.enabled=true --set components.kserve-llmisvc-crd.enabled=true \
  --set components.kserve-llmisvc-resources.enabled=true --set components.kserve-runtime-configs.enabled=true
...
Model serving on a GPU cluster is its own release of this chart (giantswarm/agent-platform#326; [bumblebee-plans#46](https://github.com/giantswarm/bumblebee-plans/pull/46), D4/D6/D8/D9): the **serving slice**, the values profile [`examples/serving-slice.yaml`](helm/agent-platform/examples/serving-slice.yaml). It renders exactly the serving component set — `kserve-llmisvc-crd`, `kserve-llmisvc-resources`, `kserve-runtime-configs` and the `modelServing` switch — and nothing of the platform's own (no muster, dicebear, valkey, kagent, Backstage, agent-manager, model-manager; the engine off). **One release per target cluster**, delivered by the installation's Flux like every workload-cluster app ([One release per target cluster](#one-release-per-target-cluster)): beside the platform's release on the installation's cluster with the profile as is, or onto a workload cluster with `gitops.target.kubeConfig.secretRef` and `components.agentgateway.enabled: true` — agentgateway is part of every installation and cannot be switched out, so nothing is detected; the operator sets the toggle by the target's shape (off beside the platform's release, which owns the controller and its CRDs; on for a workload cluster, which runs none). The profile sets `modelServing.serving.runtimeClassName: nvidia` — the RuntimeClass the GPU operator creates ([The GPU operator](#the-gpu-operator)). The GPU pool's taint and node selector come from `modelServing.gpuPool` (#315, the next PR of this lane); the predictors carry no `karpenter.sh/do-not-disrupt`: a scale-to-zero pool must be able to consolidate, and a drained predictor reloads its model from the cache PVC in seconds.

**The well-known configs.** `components.kserve-runtime-configs` (giantswarm/kserve's `kserve-runtime-configs`, `0.2.x`, `dependsOn: [kserve-llmisvc-crd]`, into the llm-d controller's lookup namespace `kserve`) installs the `LLMInferenceServiceConfig`s an `LLMInferenceService` composes from by `baseRefs` — the `llm-d-cuda` runtime image among them. The block `kserve-runtime-configs:` turns `kserve.llmisvcConfigs.enabled` on, `kserve.servingruntime.enabled` off (the chart's `ServingRuntime`s are the classic path, which the platform does not serve) and passes `kserve.llmisvcConfigs.imageRegistry: gsoci.azurecr.io/giantswarm/llm-d-fast/` — the prefix every image reference the configs render carries (the chart rewrites upstream's `ghcr.io/llm-d/` to it, giantswarm/kserve#78; nothing else names one). The prefix exists for the pull (giantswarm/agent-platform#568): containerd fetches one HTTPS stream per layer, and the byte-identical mirror's `llm-d-cuda` (8.2 GiB compressed, gzip layers of 5.5, 1.4 and 1.0 GB) was bounded by the one stream carrying its largest layer — about 60 MB/s effective, 132–149 s on a fresh pool node, where one stream measures 95 MB/s and four 306 MB/s, and the operands pulled beside it ran three to five times slower meanwhile. giantswarm/llm-d publishes the same images re-layered under `llm-d-fast/` of the same registry path — zstd, `llm-d-cuda` split into layers of at most 1.2 GB — so the pull runs over several streams. To go back, set the block's `imageRegistry` to `gsoci.azurecr.io/giantswarm/` (the byte-identical mirror) or `ghcr.io/llm-d/` (upstream) and `modelServing.prepull.images` to the matching `llm-d-cuda` reference — the pre-pull warms exactly the image the predictor runs ([Pre-pulling the runtime image](helm/agent-platform-connectivity/README.md#pre-pulling-the-runtime-image)); a private registry names its own prefix the same way. `make verify-serving-slice` holds the two in step. The block never travels to the connectivity release (`omitKeys`).

**The cache claim.** `modelServing.cache.pvc` (`hf-cache`, `100Gi`, on a gp3 class of the chart's own at 500 MiB/s and 3000 IOPS — sized from the loader's ≈ 300 MB/s and the Hub's ≈ 200 MB/s, not from the fastest tier; [The claim's StorageClass](helm/agent-platform-connectivity/README.md#the-claims-storageclass)) is applied by a `post-install,post-upgrade` hook of the connectivity release, not rendered as one of its resources (giantswarm/agent-platform#483): the claim has no consumer of its own — the first predictor that mounts it Binds it — and under a `WaitForFirstConsumer` class (kind's `standard`, AWS `gp3`) a release resource that never Binds fails Helm's wait and, with it, every release behind connectivity. Nothing waits for it, it outlives the release (`helm.sh/resource-policy: keep` states the intent), and an installation with an existing claim keeps its size and its class (a claim never shrinks; `storageClassName` is immutable; giantswarm/agent-platform#570).

**The models Gateway.** `modelServing.modelsGateway` (the slice turns it on; off by default, so a platform release without the serving slice renders none of it) is one agentgateway `Gateway` in the release namespace, `models` on `models.<cluster>.<base domain>` (`<hostPrefix>.<global.domain>`), TLS from the platform's wildcard Secret (`gatewayApi.gateway.tls.secretName`; or `tls.issuerRef.name` for a cert-manager `Certificate` of the host), the external-dns hostname on its infrastructure. KServe's ingress-gateway value is **derived** onto the `kserve-llmisvc-resources` release (`kserve.controller.gateway.ingressGateway.kserveGateway: <namespace>/models`; a differing copy of the operator's fails the render), so every `LLMInferenceService` route attaches to it, and a served model answers at

```
https://models.<cluster>.<base domain>/<namespace>/<model>/v1/chat/completions
```

**The audience rule.** One `AgentgatewayPolicy` on the Gateway (`Strict`, `strategy.inheritance: Override` — no route can weaken it) verifies the bearer against the platform's Dex (`global.identity.issuerUrl`; JWKS from the issuer's host on 443 by default, fetched by the **agentgateway controller** — whose network policy admits that host on 443 in every release that runs the controller, the platform's beside the slice and the slice's own on a workload cluster — and pushed to the data plane over xDS, no `gateway.jwksEgress` needed) and accepts the audience `dex-k8s-authenticator` only: the installation's login client, the id_token a person holds after the Dex login (kubectl, the portal). A request with it answers 200, one without 401 at the edge; the data plane strips the `Authorization` header once verified, so the model server logs no bearer. See [docs/authentication.md](docs/authentication.md), "The models Gateway".

**The presets.** Two 24 GB recipes for one L4-class GPU with tools on: `qwen3-4b-instruct` (BF16, ~8 GiB) and `qwen3-8b-fp8` (~9 GiB), `--enable-auto-tool-choice --tool-call-parser=hermes`, 16k context, `weightsGiB + overheadGiB <= 24`. Both are sized for the smallest L4 instance, a `g6.xlarge` (4 vCPU, 16 GiB): requests 2 vCPU / 10 GiB, limits 4 vCPU / 12 GiB — a Giant Swarm node of that shape reports 3.4 vCPU / 13.4 GiB allocatable and the daemonsets that follow the pool's taint take ~0.4 vCPU / 1.5 GiB more, so a preset requesting 4 vCPU / 16 GiB sat Pending for ever on a pool of `xlarge` nodes while Karpenter refused the size (giantswarm/agent-platform#502). A preset an installation adds under `modelServing.presets` for the same pool should stay within 3 vCPU / 11.9 GiB; cluster-manager's `create_node_pool` answers with what each of a pool's sizes leaves a predictor and which size hosts each published preset. Neither names an image: the predictor runs the `llm-d-cuda` the well-known config names; the preset schema's new optional `spec.template` carries `LLMInferenceService` template fields verbatim, an image there only with a stated reason. Every shipped preset's `weightsGiB` is what the Hub holds: `make verify-preset-weights` (in `verify-all`, so on every PR) sizes each preset's `spec.model.id` the way model-manager's `check_fit` does — the safetensors index's `total_size`, else the safetensors files — and fails a preset below the Hub's size, more than 15 % above it, or one the Hub cannot size, so cluster-manager's `presetFit` and model-manager's `check_fit` judge a pool from the same number (giantswarm/agent-platform#535).

**The 48 GB presets.** Three recipes for one 48 GB GPU of the L40S family (`g6e.2xlarge` or `g6e.4xlarge`; requests 4 vCPU / 32 GiB, limits 8 / 48, `--gpu-memory-utilization=0.92`, four concurrent requests, tools on, one image per prompt), each served from a signed model image (giantswarm/agent-platform#591). `qwen3-8-27b-l40s` serves Qwen3.8 27B as NVIDIA's NVFP4 checkpoint (20.4 GiB: NVFP4 on the MLPs and the language-model head, FP8 on the attention layers; the GPU has no native FP4, so vLLM takes the Marlin weight-only path for the NVFP4 layers; 64k context, thinking off by default, no speculative decoding) — the shipped `qwen3-8-27b` is the same model tuned for a 128 GB unified-memory Blackwell node (DFlash2 speculative decoding, 256k context, a patched chat template) and hosts on no curated AWS size (giantswarm/agent-platform#544). `qwen3-6-35b-a3b` serves Qwen3.6 35B-A3B, Qwen's FP8 mixture of experts with 3B active per token (35 GiB, 32k context, the `qwen3` reasoning and `qwen3_coder` tool parsers, thinking off by default). `gemma-4-31b` serves Gemma 4 31B as Red Hat's FP8-dynamic quantization (31 GiB, 64k context, the `gemma4` tool and reasoning parsers under vLLM's Gemma 4 chat template, shipped as `chat-templates/gemma-4-31b.jinja`; audio off). Meta's Muse Glimmer 30B, the line-up's fourth 48 GB model, ships no preset yet: vLLM serves its architecture and its `muse_glimmer` parsers from v0.28.0, and the platform's runtime image (llm-d v0.8.0) carries vLLM v0.23.0 — the preset joins once the well-known runtime image carries vLLM 0.28 or later. A model whose weights exceed the card it targets ships no preset: `make verify-preset-weights` sizes every shipped preset's `weightsGiB` against the Hub, and `weightsGiB + overheadGiB` stays within the card.

**The Qwen3.8-Flash-Next preset.** `qwen3-8-flash-next-nvfp4` serves Qwen3.8-Flash-Next (a 125B mixture of experts with 6B active, a 51B n-gram embedding table, 262k context, tools, reasoning, vision) on one 128 GB unified-memory Blackwell node from an OCI model image (giantswarm/agent-platform#553). NVIDIA's NVFP4 checkpoint is 123.6 GiB on disk and does not fit the node next to a KV cache, so the preset serves the quantization-aware-distilled repack `local-inference-lab/Qwen3.8-Flash-Next-NVFP4` (98.6 GiB, the n-gram table in NVFP4 too) with the table read from disk (`VLLM_PLE_TABLE_MEMORY=disk`) and MTP speculative decoding (four draft tokens): `weightsGiB: 99` (the Hub's size), `overheadGiB: 20` (about 74 GiB resident; `--gpu-memory-utilization=0.80` of the 128 GB leaves the rest to the fp8 KV cache), eight concurrent requests at 4096 batched tokens, the `qwen3` reasoning parser, tools on with the `qwen3_xml` parser. It is the first shipped preset served from a model image. Its serving image is the B12X serving stack — the upstream vLLM image cannot serve this architecture on the node — which the installation names as the runtime image of the well-known `LLMInferenceServiceConfig` such a node serves with (`kserve-runtime-configs`, [The well-known configs](#the-serving-slice-and-the-models-gateway)); the preset itself names no image. `spec.model.storageUri` is `oci://gsoci.azurecr.io/giantswarm/models/qwen3-8-flash-next-nvfp4:7c4f1bc1a2d6` (the tag is the first 12 characters of the checkpoint revision); `modelServing.modelImages.registry` swaps the host for an installation's own registry that holds the same path, and `modelServing.prepull.modelPresets` pre-pulls the image onto the serving nodes. `HOME`, `HF_HOME` and `VLLM_CACHE_ROOT` sit under `/tmp` because the modelcar path runs the runtime as a non-root uid on a read-only image. `make verify-wiring` expects eleven shipped presets; `verify-preset-weights` and the eval check of `verify-serving-slice` cover the file; `verify-gpu-pool` leaves the preset's ConfigMap and its discovery entry out of the golden comparison while `origin/main` predates it.

**One serving path.** Every served model is an `LLMInferenceService` model-manager composes from a published preset onto the well-known `LLMInferenceServiceConfig`s (giantswarm/agent-platform#574): the chart renders no `serving.kserve.io` object of its own — no `ClusterServingRuntime`, no `InferenceService` — and the platform runs one KServe controller, the llm-d one. The well-known template's container runs the argument grammar `eval "exec vllm serve … $@"`, which re-parses `$@`, so a preset's JSON-valued flag is written single-quoted inside one argument (`--limit-mm-per-prompt='{"image": 1, "video": 0}'`) and reaches vLLM as one word with the quotes gone; the render refuses a preset argument with whitespace, a quote or a shell metacharacter outside single quotes (giantswarm/agent-platform#532). The runtime image is the well-known config's (`kserve-runtime-configs.kserve.llmisvcConfigs.imageRegistry`, [The well-known configs](#the-serving-slice-and-the-models-gateway)); a model that needs another serving image — a hardware-specific vLLM build for a unified-memory Blackwell node — gets it through that chart, never through a runtime object of this one. A preset carries no `spec.runtime` and no `spec.predictor` (the classic path's fields; the render refuses them naming the preset); `spec.template` carries `LLMInferenceService` template fields. `make verify-serving-slice` runs the template's entrypoint over every shipped preset's arguments.

**Models as OCI images.** A preset whose `spec.model.storageUri` is `oci://<registry>/<path>[:tag|@digest]` is served from a **model image** — KServe's modelcar path (giantswarm/agent-platform#551): containerd pulls the image onto the node's local disk like any other, a sidecar exposes its `/models` directory to the runtime container, no storage-initializer runs and the cache claim is not mounted (the cache redirect rule selects pods with a storage-initializer and leaves a modelcar pod alone), so a warm node serves a 100 GiB model from its own disk with no download and every serving node is a target — no claim binds the model to one node or zone. The image contract: the weights under `/models` in a busybox-based image (the modelcar sidecar runs a shell, `ln`, `sleep` and `ls` from it); the runtime container runs as the platform KServe's modelcar uid (1010) on the model path, so an OCI preset sets `HOME`, `HF_HOME` and `VLLM_CACHE_ROOT` to paths under `/tmp` in its `env` — vLLM's compile cache on the claim is not available to it. A shipped OCI preset references the canonical registry, `gsoci.azurecr.io/giantswarm/models/<preset>:<revision>`; **`modelServing.modelImages.registry`** (empty by default) swaps the registry host of every `oci://` preset at render — a shipped preset's and a values preset's alike; the host is the segment before the first `/`, the path, tag or digest is kept — so an installation serves the same image from a registry of its own (a mirror, an in-cluster registry) that holds the same path. `hf://` presets are untouched; the render refuses a registry with a scheme or a path and an `oci://` reference without a host. The host is published in the discovery ConfigMap as `spec.modelImages.registry`, so model-manager builds its own references with it. **`modelServing.prepull.modelPresets`** names the presets whose images join the pre-pull DaemonSet's init containers after the runtime images — `pull-model-<preset>` on the published reference minus `oci://`, `/bin/true` — so every serving node holds the weights before the first serve and the running pause pod keeps the image against the node's image garbage collection; a name that is not a published preset, or not `oci://`, fails the render naming it.

```yaml
modelServing:
  modelImages:
    registry: registry.example.com       # oci://gsoci.azurecr.io/giantswarm/models/x:v1 is published as oci://registry.example.com/giantswarm/models/x:v1
  prepull:
    modelPresets: [my-model]             # pull-model-my-model on registry.example.com/models/my-model:7c4f1bc
  presets:
    - apiVersion: agent-platform.giantswarm.io/v1alpha1
      kind: ServingPreset
      metadata: {name: my-model}
      spec:
        displayName: My model, served from an image
        model:
          id: org/My-Model
          storageUri: oci://registry.example.org/models/my-model:7c4f1bc   # the host is swapped, the path kept
        env:
          - {name: HOME, value: /tmp}
          - {name: HF_HOME, value: /tmp/hf}
          - {name: VLLM_CACHE_ROOT, value: /tmp/vllm-cache}
        requirements: {weightsGiB: 99}
```

`make verify-model-images` holds the rewrite (shipped and values presets, `hf://` untouched, an empty registry as written, a digest kept), the init containers, the guards and the meta chart's forwarded render; `make verify-model-serving-policies` holds the pod with a model init container to the fleet's restricted Pod Security Standard with no exception.

**Verifying model images.** A model pod runs whatever the registry serves under its references — the modelcar sidecar that carries the weights, the pre-fetch init container KServe runs from the same image, the runtime container — so admission verifies them: `modelServing.imageVerification` (on by default; giantswarm/agent-platform#552, #575) renders one Kyverno `verifyImages` ClusterPolicy, `<release>-model-serving-image-verification`, over the model pods of the serving namespace — one rule per pod shape, Pods at CREATE and UPDATE (a pod's container images are mutable, so a CREATE-only rule would admit an image swapped into a running pod; Kyverno records the verified images in an annotation and skips an unchanged one) — where the cache policies render (`modelServing.policies.enabled` resolved: Kyverno served) and nowhere else. Kyverno verifies every container image of the pod that matches `images` (its `imageReferences`: a full reference or a wildcard, `*` any run of characters across path segments, tags and digests, `?` one character) against `attestors`, one entry per trusted signer, passed through as written into one attestor set with `count: 1` (a signature of any one signer admits the image): keyless (`issuer` + `subject` or `subjectRegExp`, `rekor.url`), a public key (`keys.publicKeys`, a PEM) or `certificates`. The defaults admit exactly what Giant Swarm signs: `images: [gsoci.azurecr.io/giantswarm/*]` — every image under the platform's registry namespace, the curated model images, the llm-d runtime and sidecars, the KServe storage-initializer — against one keyless attestor, the identity every image built or re-signed by a Giant Swarm CircleCI project carries (issuer `https://oidc.circleci.com`, subject the pipeline definition that ran), read as the Sigstore bundle cosign 3 attaches (`type: SigstoreBundle`; `Cosign` for a signer that writes cosign 2's `.sig` tags). A container image matching no pattern is left alone: an installation serving from a registry of its own (`modelServing.modelImages.registry`) is verified only once it names its pattern and its signer. `mutateDigest: true` replaces the verified tag with its digest, so the kubelet runs exactly what was verified (Kyverno requires a digest on a verified image; with it `false`, the preset pins one); `required: true` denies an image without a signature; `failureAction: Enforce` denies, `Audit` admits and reports. A pod the rule refuses never reaches the kubelet: the workload's ReplicaSet reports Kyverno's admission error — `verify-model-images-<shape> failed to verify image <reference>: … sigstore bundle verification failed: no matching signatures found` — and model-manager surfaces it as the model's phase. An installation whose model pods run an image under the pattern that no attestor signed — one pushed to the namespace by hand, one re-signed by a signer of its own — adds that signer or turns the switch off before it upgrades. Enabled with an empty `images` list, no attestor, an entry that is no Kyverno attestor entry, a `type` or a `failureAction` outside the two or an unknown key fails the render naming the key. A second signer beside the fleet's, keyless:
  ...
  imageVerification:
    ...
    attestors:
      ...
      - keyless:
          ...
          subject: https://github.com/example/models/.github/workflows/publish.yaml@refs/heads/main
          rekor:
            url: https://rekor.sigstore.dev
```

The meta chart mirrors the block (`make verify-meta` holds every leaf equal); `make verify-model-serving-policies` asserts the rendered rules of the default render — the match of each shape, the references, the attestor entry and the knobs at their defaults —, an installation's own block reaching every rule verbatim, runs the policy through `kyverno apply` over both fixture pods (none of whose images match: every rule skipped, the policy accepted by Kyverno's schema, a misspelt field refused), nothing with the switch off or without Kyverno, the guards, and the meta chart's forwarded render equal. A signature itself is checked against the registry and Rekor, not offline: the proof is `kyverno apply` of the rendered default policy over a pod naming every image a shipped preset's pod runs — mutated to its digests — and a pod naming an unsigned image denied, on a cluster's Kyverno.
...
| `components.modelServing.enabled` (+ `components.kserve-llmisvc-crd`, `components.kserve-llmisvc-resources`, `components.kserve-runtime-configs`) | Model serving on llm-d (the `modelServing:` block reaches the connectivity release only while the switch is on): the serving presets (one ConfigMap per preset, label `agent-platform.giantswarm.io/serving-preset=true`; seven shipped under `files/model-serving/presets/`, more or replacements under `modelServing.presets`), the discovery ConfigMap `agent-platform-model-serving`, the serving `Namespace`, the Hugging Face cache `PersistentVolumeClaim` (kept on uninstall), the two Kyverno `ClusterPolicy` objects that wire every predictor pod to the cache (`modelServing.policies.enabled: auto` — where `kyverno.io/v1` is served), and the network policies of the serving namespace in the resolved flavor. The render fails with a clear message when neither the two KServe components are on nor the `serving.kserve.io` APIs are served (`modelServing.kserve.requireApi`). A model-manager on the `kserve` backend must agree with the layer (`model-manager.kserve.*`; the chart's defaults do). | `modelServing.*` — the standalone's `components.modelServing.*` block, one-to-one |
| `components.kserve-llmisvc-resources.enabled` | The llm-d controller's network policy (webhook 9443, metrics 8443, probes 8081; DNS and API egress) in both flavors, and the guards: `kserve-llmisvc-resources.kserve.controller.deploymentMode` must be `Standard`, `kserve-llmisvc-resources.kserve.createSharedResources` must stay `true` (the release renders the control plane's shared objects), and the removed classic components `components.kserve-crd` / `components.kserve-resources` are refused naming them. | the component chart's own block |
...
| `components.kserve.enabled` | gone with the classic `InferenceService` path: the platform serves through llm-d alone (`components.kserve.llmisvc.enabled` below); a values file that still carries `components.kserve-crd` or `components.kserve-resources` fails the render naming them |
...
| `components.kserve.certManager.requireApi`, `components.kserve.llmisvc.requireApi`, the two-phase first install | gone: `dependsOn` orders the CRD charts before the controllers and both before the connectivity and model-manager releases; cert-manager stays a documented prerequisite of `kserve-llmisvc-resources` |
...
| `kagent.kagent.*`, `agentgateway.agentgateway.*` (nested subcharts) | `kagent.*`, `agentgateway.*` (keys at the chart root: the kagent line's chart, the flattened 2.x agentgateway chart) |
...
| `mcp-kubernetes.*`, `backstage.*` (the chart blocks), `kserve-llmisvc-resources.*` | unchanged |
...
| `agenttemplates.kagent.dev`, `harnesses…`, `modelconfigs…`, `modelproviderconfigs…`, `remotemcpservers…` | the **kagent-crds** component chart (the kagent line, `giantswarm/kagent-upstream`) — as templates, not `crds/`; `keep`-annotated. Follows `components.kagent`. |
| `workerpools.ate.dev`, `sandboxconfigs.ate.dev`, `csidriverconfigs.ate.dev` | the **substrate-crds** component chart (the Substrate line, `giantswarm/substrate`) — as templates, not `crds/`; `keep`-annotated (from `v0.0.27-gs.3`). Follows `components.kagent`; the kagent release's `WorkerPool` and the substrate release's `SandboxConfig` are its CRs. |
...
| `llminferenceservices.serving.kserve.io`, `llminferenceserviceconfigs…`, `clusterstoragecontainers…` | the **kserve-llmisvc-crd** component chart (`giantswarm/kserve`) — as templates, not `crds/`; `keep`-annotated. Off by default. The classic `inferenceservices…`, `servingruntimes…` and `clusterservingruntimes…` CRDs are not installed by the platform any more (giantswarm/agent-platform#574). |
...
Each component that ships a `crds/` dir sets `crds: CreateReplace` on its `HelmRelease` (rendered by the meta-package), so Flux applies and upgrades those CRDs atomically with the app at the same resolved version — Helm on its own never upgrades `crds/`-dir CRDs. The kagent-crds, substrate-crds, kserve and cloudnative-pg charts render their CRDs as ordinary templates instead, so their `HelmRelease`s carry no `crds:` policy and Helm applies and upgrades them with every release. All these CRDs carry `helm.sh/resource-policy: keep` (CNPG's through the operator chart's own handling), so they survive a component uninstall (the CRs are never cascade-deleted).
...
The connectivity chart renders `kyverno.io` objects: the agent-sandbox pod-security ClusterPolicy; with Agent Substrate on, the four PolicyExceptions of Substrate's workloads ([Agent Substrate](#agent-substrate)); and, only with `postgres.vector.extensionImage.reference` set, the PolicyException that admits the CNPG instance pods' `image` volume. (The two ClusterPolicies that gave the v1alpha2 declarative agents their securityContext and their srt settings went with kagent API v2 — agents run as Substrate actors, so there is no `Agent` CR, per-agent Deployment or config Secret left to mutate — and so did those agents' seccomp exception; `substrate-workers` is its successor.) On a cluster with no Kyverno they would fail the install on an unknown API group, so `kyvernoPolicies.enabled: auto` (the default) renders none of them where `kyverno.io/v1` is not served; `false` forces that, `true` forces them on.

A PolicyException targets policies this chart does not own, so the names must match the target cluster. `kyvernoPolicies.rules` maps every rule an exception may name to its ClusterPolicy — the upstream kyverno-policies names of the baseline and restricted Pod Security Standard policies Giant Swarm enforces by default (`host-path: disallow-host-path`, `check-seccomp-strict: restrict-seccomp-strict`, …); each exception cites the rule and its `autogen-<rule>` copy for the controller kind. A cluster that names its policies differently overrides the map; a rule with no entry fails the render. `policyExceptionNamespace` (default `policy-exceptions`) is where the exceptions live.
...
**Render assertions** (no cluster; `test-ingress-modes`, Helm 3.17.3): `make verify-modes verify-global verify-meta verify-engine verify-self verify-managers verify-kagent-netpol verify-kagent-discovery verify-kagent-crds verify-postgres verify-secrets verify-presets verify-components verify-components-charts verify-auto verify-identity verify-wiring verify-migration verify-kagent-storage-version` — `Makefile.custom.mk` describes each target. `verify-meta` also walks the two `values.schema.json` files against each other, nested keys included (`tests/verify-schema-symmetry.py`): every key the connectivity chart declares is settable through the meta chart, and every key the meta chart forwards is declared by the connectivity chart — a key missing on either side fails a render, at render time here or on the connectivity release on every installation. The three shapes of the chart map onto them: engine on + self on (the quick start) is `verify-self`'s; engine on + self off (the lab shape, the hand-back) is `verify-engine`'s and `verify-self`'s; engine off (the fleet: `--include-crds` renders no CRD, hook, operator or self object — the pure renderer) is `verify-engine`'s and `verify-meta`'s; the `auto` knobs — the vanilla shape without `--api-versions`, the fleet shape with the API groups passed — are `verify-auto`'s. `pre-commit run -a` regenerates the schemas and the helm-docs READMEs.

**The ATS** (`execute-chart-tests`, a required check; [tests/ats/README.md](tests/ats/README.md)) runs two scenarios on one kind cluster on a 4-vCPU `large` CircleCI machine — a cluster created from [`.ats/kind-config.yaml`](.ats/kind-config.yaml) with the `ClusterTrustBundle`, `ClusterTrustBundleProjection` and `PodCertificateRequest` feature gates Agent Substrate needs: the **smoke** — the quick start on a bare cluster (the Gateway API CRDs, a lab Dex and an in-cluster registry the candidate is pushed to, together with the connectivity chart of the checkout, which the roster is pointed at through `components.agent-platform-connectivity.{repository,versionRange,insecure}`; `helm install --wait` with the bundled engine, muster with its OAuth server, dicebear, connectivity, kagent with its CRD chart, Substrate with its CRD chart, agent-manager, self-management on against that registry), then the adoption, the auth round trip (the 401 discovery chain, a Dex user through the password grant and through muster's login flow), Substrate from the chart under test (its WorkerPool's gVisor worker Ready, the platform Harness), the agent round trips on kagent API v2 (a declarative `AgentTemplate` Ready on the platform Harness; agent-manager's `create_agent` through muster as the Dex user → the chart `1.x` HelmRelease as `kagent-flux` → the `AgentTemplate` Ready on the Harness and the agent's `RemoteMCPServer` Accepted), the drift correction (the platform Harness deleted by hand and back on the kagent release's next reconcile — a requested one, no `forceAt`, no Helm revision — with both templates Ready on it again), the self-management fixpoint and values Secret, the refused `helm upgrade`, and the ordered teardown with the line's keep policy (the `kagent-crds` release uninstalled, its CRDs and the agents' templates surviving, nothing else) — and the **own-Flux scenario**: Flux's source-controller and helm-controller from the upstream manifest, the chart through a `HelmRelease` with `components.flux.enabled: false` (no operator, no second helm-controller, the Flux CRDs' field managers untouched, an agent through that Flux on the platform Harness) and the render guard when the value is flipped on. The smoke found the fresh-install deadlock on the kagent namespace fixed in this release.

Measured (each test logs `TIMING <phase>`; CI = the 4-vCPU `large` machine with cold pulls, CircleCI job 6033 of PR #380; the per-phase table with the Substrate sizing is in [tests/ats/README.md](tests/ats/README.md#the-cluster-kind-with-agent-substrate)): **smoke** — `helm install --wait` 220 s, Substrate ready 0 s after the install returned (the install already waited for kagent behind it), the declarative `AgentTemplate` Ready on the Harness 20 s, agent-manager `create_agent` → HelmRelease Ready → `AgentTemplate` Ready + `RemoteMCPServer` Accepted 16 s, the fixpoint wait fills the rest of two 1-minute intervals after the adoption, `helm upgrade` refused in 1 s, `helm uninstall --wait` 31 s (five waves; the kagent namespace is kept); the smoke's pytest run 6:58. **functional** — the chart through the cluster's Flux 289 s, the agent through that Flux 36 s, the guard firing 51 s after the flip, the way back 105 s; the pytest run 8:54. The whole `execute-chart-tests` job: **16:55** (kind create 36 s, the ATS image pull and `uv sync` ~1 min, the two pytest runs the rest). The long pole is the two platform installs — every component image pulled once; Substrate's control plane, the worker image and the Go ADK image the Harness boots are the additions of kagent API v2.
