...
**Additional runtimes.** `modelServing.additionalRuntimes` renders more `ClusterServingRuntime`s beside `kserve-vllm` from the same template (giantswarm/agent-platform#550) — for the models whose accelerator needs a different serving image, a hardware-specific vLLM build for a unified-memory Blackwell node next to the upstream image the other presets run on. Each entry needs a `name` of its own (a DNS-1123 subdomain; the render refuses one already used, naming the entry) and takes the default runtime's value for every field it leaves unset: `image` (registry, name, version), `args`, `env`, `resources`, `shmSize`, `startupProbe`, `annotations`, `supportedModelFormats`, `nodeSelector`, `tolerations` — a mapping field by field, a list whole (an entry's `env` replaces the default's two entries rather than extending them), and an empty value inherits too (Helm's merge). The GPU pool's taint and label apply to every runtime as to the default ([The GPU node pool](helm/agent-platform-connectivity/README.md#the-gpu-node-pool)); the container runs through the same shell entrypoint with the entry's own base arguments, so its image must carry `/bin/sh` and `vllm` on `PATH` as well. A preset selects a runtime with `spec.runtime`. The discovery ConfigMap publishes every runtime's name as `spec.runtimes` (the default first) next to `spec.runtime`; an empty list renders the default alone. Mirrored in the meta chart (`make verify-meta` holds the two `runtime` and `additionalRuntimes` blocks equal — an additional runtime inherits from the forwarded default); `make verify-runtimes` asserts the render over the fixture `ci/test-model-serving-runtimes-values.yaml`.
