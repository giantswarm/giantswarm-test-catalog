...
## Meta-package release flow (POC)

> Proof of concept for [giantswarm/giantswarm#36875](https://github.com/giantswarm/giantswarm/issues/36875). Concept write-up lives in the klaus-lab (`architecture/agentic-platform-meta-package.md`).

This chart no longer bundles its components as pinned Helm subcharts. It is an
**app-of-apps meta-package**: `templates/components.yaml` renders, per component
(muster, agentgateway, kagent, klaus-gateway, valkey, agentic-platform-mcps, and
agentic-platform-crds), a Flux `OCIRepository` + `HelmRelease` — or an Argo
`Application` when `gitops.engine: argo`.

The decisive change: each component's version is a **constraint expressed as a
value** (`components.<name>.versionRange`), not a `Chart.yaml` pin. Flux
re-resolves the range on every reconcile, so a new component release rolls
forward **with no PR to this chart and no umbrella re-package**.

- **On/off and per-component values are unchanged** — the existing `muster:`,
  `agentgateway:`, `valkey:`, `kagent:`, `klausGateway:`, `agentic-platform-mcps:`
  blocks and `*.enabled` toggles still drive each component. Each `components.<name>`
  entry names its source block explicitly via `valuesFrom` (and its toggle via
  `enabledFrom`); the named block is passed straight through to the rendered
  release (`HelmRelease.spec.values` / Argo `valuesObject`), with the umbrella
  `global` injected as before. The blocks stay at top level so existing override
  paths and `values.schema.json` are untouched.
- **CRD-before-CR ordering is preserved** — `agentic-platform-crds` renders as a
  `HelmRelease` that the component releases `dependsOn` (Flux) / order after via
  `argocd.argoproj.io/sync-wave` (Argo).
- **Integration/glue layer is unchanged** — `Gateway`, `AgentgatewayParameters`,
  `HTTPRoute`s, `NetworkPolicy`s and the `mcpServers` list remain plain templates
  in this chart, re-released only when the wiring shape changes.
- **Dev vs customer track** — keep the `components.*.versionRange` values wide for
  the internal/dogfooding track (continuous auto-update). Pin them to exact
  versions for a customer **bill-of-materials**; see
  [`examples/customer-bom.yaml`](helm/agentic-platform/examples/customer-bom.yaml).
  A "product release" is that pinned values snapshot.

```bash
helm template r helm/agentic-platform --set ingress.parentRefs[0].name=x          # flux objects, wide ranges
helm template r helm/agentic-platform --set ingress.parentRefs[0].name=x --set gitops.engine=argo
helm template r helm/agentic-platform --set ingress.parentRefs[0].name=x -f helm/agentic-platform/examples/customer-bom.yaml
make verify-meta
```

**Known caveat (discussion input):** the glue-layer CRs are applied by this
chart's own release, while the CRDs arrive via the `agentic-platform-crds`
HelmRelease. On a first install the CRs may briefly precede the CRDs until the
crds release reconciles — same prerequisite as today (CRDs first), now expressed
through `dependsOn` / sync-waves. Flagged for the honeybadger onsite.
