...
**The cache claim.** `modelServing.cache.pvc` (`hf-cache`, `500Gi`, the cluster's default StorageClass) is applied by a `post-install,post-upgrade` hook of the connectivity release, not rendered as one of its resources (giantswarm/agent-platform#483): the claim has no consumer of its own — the first predictor that mounts it Binds it — and under a `WaitForFirstConsumer` class (kind's `standard`, AWS `gp3`) a release resource that never Binds fails Helm's wait and, with it, every release behind connectivity. Nothing waits for it, it outlives the release (`helm.sh/resource-policy: keep` states the intent), and the default class of the fleet needs no special handling.
