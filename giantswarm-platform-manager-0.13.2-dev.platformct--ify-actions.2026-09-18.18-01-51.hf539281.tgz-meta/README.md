...
- `platformctl installation list [<installation>…] [--customer <name>]`,
  ...
  `--dry-run` is the tool's `dryRun`; `--commit` its `mode: commit` for one installation — the pull
  requests opened as you, the Team review asked. `--secret <field>=@<file>`, `<field>=env:<NAME>` or
