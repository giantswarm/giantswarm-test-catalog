...
1. **The opt-in gate** — the one condition the manager checks itself, and it protects what is on record:
   a commit that would change the files of a capability on record — installed by the owners themselves, or
   by the manager before they withdrew their declaration — needs `management-clusters/<name>/platform-manager.yaml`
   with `optIn: true` in the installation's management-clusters repository, read as the person at call time,
   never cached. Absent, `optIn: false` or unreadable, the commit is refused naming the capability, the
   installation and the file, and the refusal is recorded as an Action in state *refused* (the installation's
   state read from its repositories stands). A fresh enable — nothing of the capability on record — needs no
   declaration: it writes nothing of the owners', and its pull requests in their repositories are the review.
   A withdrawn opt-in stops the next change to what is on record.
...
`commitRefused` saying why a commit would be refused — the capability on record without the owners' opt-in,
and how they opt in — and empty for a fresh enable. `order` names another
...
there; `optedIn`: the owners have declared the opt-in, the manager may change what is on record — and
their state as one word: *not enabled* (marker absent, opted in or not: the manager may enable it, the
pull requests in the owners' repositories being the review), *enabled, not opted in* (marker present, no
declaration or `optIn: false`: the owners enabled the capability themselves, before the manager existed
or beside it, and the manager may not change its files until they opt in) or *enabled* (marker present,
opted in). *Pending approval*, *rolling out*, *waiting for the customer*,
...
The installation's owners' standing consent that the manager may change the platform capabilities
on record — the files of a capability they installed themselves, or that the manager installed
before — with pull requests as the person, alone or in a wave. It is landed by the owners' own pull
request in the installation's management-clusters repository — never by this manager, which would be
consenting on their behalf — at `management-clusters/<name>/platform-manager.yaml`, read at call time
and never cached. Absent or `optIn: false`, the installation is *not opted in*: a capability on record
is not changed, a wave skips the installation, and every answer names the file's path and the pull
request that would add it. A fresh enable of a capability not on record needs no declaration. The
file is not in the directory's `kustomization.yaml`: Flux never applies it.
...
```json
{
  ...
  "properties": {
    "optIn": {
      "type": "boolean",
      "description": "true: the manager may change this installation's platform capabilities on record with pull requests as the person calling it, alone or in a wave. false or absent: a capability on record is read, never changed, and a wave skips the installation; a fresh enable of a capability not on record needs no declaration."
    }
  }
}
```
