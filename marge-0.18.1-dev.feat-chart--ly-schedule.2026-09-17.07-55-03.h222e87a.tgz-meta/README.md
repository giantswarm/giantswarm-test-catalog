...
- `--all-teams` sweeps every team that has a policy file, each under its own repositories and its own [policy](#sweep-policy). A team whose policy sets `schedule: disabled`, and a team with no policy file at all, are both skipped and named in the report. One team's unreadable policy fails that team alone; every other team still runs, and the command exits non-zero at the end. This is what the daily schedule runs.
  ...
  Teams are swept concurrently, `--team-concurrency` at a time (4 by default), and the PRs of the teams that overlap share one limiter of 20. The ceiling belongs to the GitHub token, not to a team, so running more teams at once never puts more on the token than one team may put on it alone. `--team-timeout` (30m by default) bounds one team, so a team that hangs cannot take the whole run and leave the teams after it unreported. The result is reported in team order however the runs interleave.
...
| `--team-concurrency` | | `4` | How many teams `--all-teams` sweeps at once; overlapping teams share one limiter |
| `--team-timeout` | | `30m` | How long one team of `--all-teams` may take before its sweep is cut short |
